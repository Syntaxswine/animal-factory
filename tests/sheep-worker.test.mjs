...
test('scarf ends have outward-facing volume and clear the actual shirt and vest in both meshes',()=>{
 for(const data of [author,reduced]){
  const cloth=data.parts.filter(p=>/shirt|waistcoat/.test(p.name)).map(p=>new THREE.Mesh(geometry(p),new THREE.MeshBasicMaterial({side:THREE.DoubleSide})));
  const ray=new THREE.Raycaster();
  for(const part of data.parts.filter(p=>p.name.includes('neckerchief end'))){
   let volume=0;const p=part.position,edges=new Map();
   for(let i=0;i<part.index.length;i+=3){const a=new THREE.Vector3().fromArray(p,part.index[i]*3),b=new THREE.Vector3().fromArray(p,part.index[i+1]*3),c=new THREE.Vector3().fromArray(p,part.index[i+2]*3);volume+=a.dot(b.cross(c))/6;for(let j=0;j<3;j++){const a=part.index[i+j],b=part.index[i+(j+1)%3],key=Math.min(a,b)+':'+Math.max(a,b);edges.set(key,(edges.get(key)||0)+(a>b?1:-1));}}for(const count of edges.values())assert.equal(count,0,'adjacent cloth faces have consistent winding');
   assert.ok(volume>1e-6,'closed cloth has positive volume and outward winding');
   for(let i=0;i<p.length;i+=3){ray.set(new THREE.Vector3(.5,p[i+1],p[i+2]),new THREE.Vector3(-1,0,0));const hit=ray.intersectObjects(cloth)[0];assert.ok(hit);const clearance=p[i]-hit.point.x;assert.ok(clearance>.001&&clearance<.040,`cloth clearance ${clearance}`);}
  }
  cloth.forEach(m=>{m.geometry.dispose();m.material.dispose();});
 }
});
