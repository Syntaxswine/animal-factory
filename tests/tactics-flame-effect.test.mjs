...
const TAU=Math.PI*2;
// A Canvas2D stand-in that records every call, refuses non-finite numbers, and flattens each filled
// path into polygons (arcs, ellipses and curves sampled) so their winding can be checked.
function recorder(){
 ...
 const ctx={calls,fills,
  ...
  closePath(){calls.push(['closePath']);if(current){current=null;}},
...
const area=polygon=>polygon.reduce((sum,[x,y],i)=>{const [u,v]=polygon[(i+1)%polygon.length];return sum+x*v-u*y;},0)/2;
