...
import {createCowWorker} from '../dist/tactics/cow-worker.js';
...
const author=load('cow-author-data.json'),reduced=load('cow-10k-data.json');
...
for(const data of [author,reduced]){
 test(`${data.triangles}: closed connected surfaces, scale and species landmarks survive reduction`,()=>{
  ...
  const h=createCowWorker(data);try{const d=h.diagnostics();assert.ok(Math.abs(d.min[1])<1e-6);assert.ok(Math.abs(d.max[1]-1.60)<.002);
   const skull=h.parts[6].geometry.attributes.position;let ears=[0,0],crown=0;for(let i=0;i<skull.count;i++){const y=skull.getY(i),z=skull.getZ(i);if(Math.abs(z)>.17&&y>1.43&&y<1.57)ears[z<0?0:1]++;if(y>1.58){crown++;assert.ok(Math.abs(z)<.065,'hornless crown has no lateral horn tips');}}assert.ok(ears.every(n=>n>10),'broad lateral ears survive');assert.ok(crown>8,'short forelock survives');
   const tail=h.parts[7];assert.match(tail.name,/tail/);tail.geometry.computeBoundingBox();assert.ok(tail.geometry.boundingBox.min.y<.27);assert.ok(tail.geometry.boundingBox.max.y>.90);assert.ok(tail.geometry.boundingBox.max.x<-.15,'tail attached at rear pelvis');const si=tail.geometry.attributes.skinIndex,sw=tail.geometry.attributes.skinWeight;for(let i=0;i<si.count;i++){assert.equal(si.getX(i),0,'tail follows hips');assert.equal(sw.getX(i),1);}   const ray=new THREE.Raycaster();for(const side of [-1,1]){const hoof=h.parts.find(p=>p.name==='exposed cloven hoof '+side),x=[];for(const dz of [0,-.035,.035]){ray.set(new THREE.Vector3(.4,.04,side*.232+dz),new THREE.Vector3(-1,0,0));const hit=ray.intersectObject(hoof)[0];assert.ok(hit);x.push(hit.point.x);}assert.ok(x[1]-x[0]>.035&&x[2]-x[0]>.035,'actual geometry has two separated toes');}
  }finally{h.dispose();}
 });
 test(`${data.triangles}: normalized rig, fixed feet, independent rifle grips and neutral restoration`,()=>{const h=createCowWorker(data);try{assert.equal(h.bones.length,17);for(const m of h.parts){const a=m.geometry.attributes;for(let i=0;i<a.position.count;i++){let sum=0;for(let k=0;k<4;k++){const w=a.skinWeight.array[i*4+k];assert.ok(w>=0&&w<=1);sum+=w;}assert.ok(Math.abs(sum-1)<1e-6);const p=new THREE.Vector3().fromBufferAttribute(a.position,i);assert.ok(m.applyBoneTransform(i,p.clone()).distanceTo(p)<1e-6);}}
