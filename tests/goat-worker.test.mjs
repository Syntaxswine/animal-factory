...
import {createGoatWorker} from '../dist/tactics/goat-worker.js';
...
const author=load('goat-author-data.json'),reduced=load('goat-10k-data.json');
...
for(const data of [author,reduced]){
 test(`${data.triangles}: closed connected surfaces, scale and species landmarks survive reduction`,()=>{
  assert.ok(data.triangles<=(data===author?30000:10500));assert.equal(data.parts.length,8);
  for(const p of data.parts){const g=geometry(p);assert.equal(surfaceComponents(g),1,p.name);const edges=new Map();for(let i=0;i<p.index.length;i+=3)for(let j=0;j<3;j++){const a=p.index[i+j],b=p.index[i+(j+1)%3];assert.notEqual(a,b);const key=Math.min(a,b)+':'+Math.max(a,b);edges.set(key,(edges.get(key)||0)+1);}for(const n of edges.values())assert.equal(n,2,p.name);assert.ok(p.errorWorld<.003);g.dispose();}
  const h=createGoatWorker(data);try{const d=h.diagnostics();assert.ok(Math.abs(d.min[1])<1e-6);assert.ok(Math.abs(d.max[1]-1.65)<.002);
   const skull=h.parts[6].geometry.attributes.position,horn=[[],[]];for(let i=0;i<skull.count;i++)if(skull.getY(i)>1.585&&skull.getX(i)<-.04)horn[skull.getZ(i)<0?0:1].push([skull.getX(i),skull.getY(i),skull.getZ(i)]);for(const side of horn){assert.ok(side.length>20);assert.ok(Math.min(...side.map(p=>p[0]))<-.24,'backswept tips');assert.ok(Math.max(...side.map(p=>p[1]))>1.64);}
 ...
 test(`${data.triangles}: normalized rig, fixed feet, independent rifle grips and neutral restoration`,()=>{const h=createGoatWorker(data);try{assert.equal(h.bones.length,17);for(const m of h.parts){const a=m.geometry.attributes;for(let i=0;i<a.position.count;i++){let sum=0;for(let k=0;k<4;k++){const w=a.skinWeight.array[i*4+k];assert.ok(w>=0&&w<=1);sum+=w;}assert.ok(Math.abs(sum-1)<1e-6);const p=new THREE.Vector3().fromBufferAttribute(a.position,i);assert.ok(m.applyBoneTransform(i,p.clone()).distanceTo(p)<1e-6);}}
