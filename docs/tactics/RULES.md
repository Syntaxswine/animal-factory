...
**Archetypes (G3, 2026-09-17).** In the campaign every guard draws one of twelve Jungian archetypes from the map seed (Innocent, Everyman, Hero, Caregiver, Explorer, Rebel, Lover, Creator, Jester, Sage, Magician, Ruler) and the four mercs carry theirs (Ruler, Rebel, Creator, Caregiver). Traits scale the alert states: a Sage hears footsteps 1.4× as far and keeps a trail four rounds, an Explorer investigates sixteen steps, a Hero breaks only at a tenth of its health and is back in one round, an Innocent breaks under half. Guards bark in their archetype's voice. Bonds between mercs sit on six rungs (bonded, trusted, cautious, strained, resented, feud) that scale friendly-fire retaliation from never to double, and rest pulls each bond back toward its resting level. Plain `createGame` keeps the base numbers (the knob is `{social:true}`); GUARDS.md G3 has the tables.
