...
**Shouts and guard-on-guard incidents (G4, 2026-09-18, campaign only).** A guard going Alert shouts once: colleagues within its archetype's shout radius (Ruler 20, Hero 16, Caregiver 14, Everyman and Jester 12, Lover and Creator 10, Innocent and Sage 8, Explorer and Magician 6, Rebel 0) that heed it take its fix and go Alert, and shout in turn; the rest go Suspicious toward the shouter. Heeding: a resented or feud listener ignores it, a bonded one always answers, anyone else rolls its obedience trait on the social stream; a Jester's listeners are never more than Suspicious. A guard hit by a colleague's bullet keeps the same ledger as a merc (stress, grudge, bond loss, a line in register, the same rung-scaled retaliation roll), and a guard whose trusted or bonded colleague falls within thirty tiles takes 15 or 25 stress and breaks on the spot when its stress passes its nerve. A plain `createGame` (the tests, the balance tool without `--social`) has none of this.
