...
- Real-time routes are bounded: one A* of at most 1,200 expansions per guard, at most three fresh searches per tick (the rest wait a tick), remembered and followed step by step until blocked or the fix changes; when the budget runs out the guard walks to the closest tile it explored and searches again from there; a fix it cannot approach at all is retried every 40 ticks. A fix on void, water or off the map (reports snap to a 6-tile grid) is replaced by the nearest walkable tile within six. On the 36-guard playtest map with every guard alert a tick costs about 100 ms worst, 60 ms mean (the first build cost 26–117 s: hostile review round 1).
