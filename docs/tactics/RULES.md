...
**Alert states (G2, 2026-09-17).** Every guard is in one of six states: Rest at its post; Suspicious after a footstep or glimpse (walks up to 12 steps toward the sound, 18 once wary, sweeps two quarter turns, stands down); Alert (today's combat routine, closing on the freshest fix; beyond two-turn reach it closes in real time); Searching after three rounds without identifying anyone, or after the real-time sweep at its fix (visits up to four neighbouring 6-tile cells, never fires, drops back to Alert on an identification); Stand-down (walks back to its post and rests, wary: it then hears footsteps half again as far); Broken when a hit leaves it at a third of its health or less (runs from the shooter toward an ally or its post for two rounds, fires only when cornered, then returns to Alert if it still knows where you are). Contact, and turn mode with it, needs a guard in Alert; Searching and Broken guards still hold the live-AP economy ("Area quiet") until they stand down ("Area clear"). A map you leave settles by the campaign clock on your return, one round per ten minutes, so the shortest there-and-back across a border (two hours) always finds its guards at post and wary. Barks reach the log within thirty tiles. The rules, constants and acceptance checks are in GUARDS.md G2.

**Archetypes (G3, 2026-09-17).** In the campaign every guard draws one of twelve Jungian archetypes from the map seed (Innocent, Everyman, Hero, Caregiver, Explorer, Rebel, Lover, Creator, Jester, Sage, Magician, Ruler) and the four mercs carry theirs (Ruler, Rebel, Creator, Caregiver). Traits scale the alert states: a Sage hears footsteps 1.4× as far and keeps a trail four rounds, an Explorer investigates sixteen steps, a Hero breaks only at a tenth of its health and is back in one round, an Innocent breaks under half. Guards bark in their archetype's voice. Bonds between mercs sit on six rungs (bonded, trusted, cautious, strained, resented, feud) that scale friendly-fire retaliation from never to double, and rest pulls each bond back toward its resting level. Every new campaign draws its own roster seed and every map hashes its name into the draw, so the yard's guards are not the factory's. Plain `createGame` keeps the base numbers (the knob is `{social:true}`); GUARDS.md G3 has the tables.

**Shouts and guard-on-guard incidents (G4, 2026-09-18, campaign only).** A guard going Alert shouts once: colleagues within its archetype's shout radius (Ruler 20, Hero 16, Caregiver 14, Everyman and Jester 12, Lover and Creator 10, Innocent and Sage 8, Explorer and Magician 6, Rebel 0) that heed it take its fix and go Alert, and shout in turn (without their own bark: one bark per cascade); a resting or stood-down colleague that does not heed goes Suspicious toward the shouter, a suspicious or searching one keeps its trail; each cascade asks a guard once. Heeding: a resented or feud listener ignores it, a bonded one always answers, anyone else rolls its obedience trait on the social stream; a Jester's listeners are never more than Suspicious. Guards keep the mercs' stress meter under the knob (any hit adds 10 plus 35 × the share of health lost); a guard hit by a colleague's bullet keeps the same ledger as a merc (grudge, bond loss, a line in register, the same rung-scaled retaliation roll), and a guard whose trusted or bonded colleague falls within thirty tiles takes 15 or 25 stress and breaks on the spot when its stress passes its nerve, away from the killer or from where the colleague fell. A plain `createGame` (the tests, the balance tool without `--social`) has none of this.
...
- Supply piles authored on the map and items comrades drop are open as before; a drop made while standing on a body goes to an open pile on that tile, never into the closed body. A bare-handed guard's "hands" are not an item and never appear. Taking from an open pile and handing items to an adjacent comrade remain free and conserve items and loaded rounds. Backpack capacity rules are unchanged.
...
- Contact is a pending casualty or fire, the squad having opened fire this turn (see below), or an alert guard that **threatens**: within two of its own turns of walking (`2 × maxAp` of its stance's movement over the real route graph, doors it would open, occupied tiles avoided) it can stand on a tile from which some squad member is inside its weapon's range with a clear line of fire; a melee guard needs an adjacent tile with an open edge between. Walls and detours count because the estimate walks the same graph as movement and traces the same rays as shooting. A guard already in range with a clear line is a threat at zero steps.
- An alert guard beyond that reach is no fight yet: the map stays in (or returns to) real time and the guard closes on its last fix one step per tick, opening doors and walking into fire like any step. Suspicious guards walk their search budget the same way; every guard with something to look into moves each tick, not one at a time. The fight opens the moment one of them could arrive within two turns. Both sides walk one tile per tick in real time, so a squad that keeps walking away is not caught; the fight opens when it stops, turns, or is cut off.
- Real-time routes are bounded: one A* of at most 1,200 expansions per guard, at most three fresh searches per tick (the rest wait a tick), remembered and followed step by step until blocked or the fix changes; when the budget runs out the guard walks to the closest tile it explored and searches again from there; a fix it cannot approach at all is retried every 40 ticks (a detour longer than the budget counts as cannot approach: the guard sweeps and stands down where an unbounded search would have routed around). A fix on void, water or off the map (reports snap to a 6-tile grid) is replaced by the nearest walkable tile within six. On the 36-guard playtest map with every guard alert a tick costs about 100 ms worst, 60 ms mean (the first build cost 26–117 s: hostile review round 1).
- Alert decays: an alert guard that reaches its fix and identifies nobody sweeps on the spot (a quarter turn per tick for eight ticks) and stands down ("gave up the search"); one that cannot approach its fix does the same. The travel marker, rest and the free real time return with it. Suspicion (footsteps) still ends when its search budget runs out.
- The warning is logged once per newly alerted guard that cannot reach the squad, whatever the phase did: "You are pretty sure someone heard that." for a report, "You have been seen, but they are too far to reach you yet." when the alert came from a sighting.
- AP is live across the engagement. While any guard on the map is alert (or the squad has opened fire this turn) every action other than walking costs its combat AP, in turn mode and in real time alike: reload 3, stance 2, equip from the backpack 3, cutting 4, stabilizing, crossing an edge, searching a body. Nothing refills AP in real time; only the end of a guard phase (a new round) does. So leaving contact mid-turn ("Area quiet") keeps whatever AP each member has, and the fight resuming in the same alert continues with it (the round counter still advances). A drop during the guard phase ends that round: the squad's next turn is a fresh one. When no guard is alert any more ("Area clear") real time is free again, overwatch reservations are cleared, and the next fight starts with a full turn. Entering another map is a fresh fight (`freshFight`, whatever alert the map's guards kept; a crosser arriving into a live contact is still capped by the AP it carried, as the retreat rule says).
- Opening fire from real time is contact: any squad attack sets the engagement, the turn opens (CONTACT), the shot is re-checked against the AP that turn actually has (the same 12 AP buy the same two rifle shots as in a turn), and the turn holds until the squad ends it; the guard phase then runs and, if nobody can reach the squad, real time resumes. A distant guard who heard the shot still gets its warning.
- Timed consequences are untouched by the mode change: bleeding and recovery tick at squad-turn ends, and a pending casualty or fire keeps turn mode on regardless of guard reach, so real time can never skip a timer.

Checks: `tests/tactics-pacing.test.mjs` (ten cases: a shot from real time opens a turn, warns the distant listener once, and the map goes quiet after the guard phase; real-time fire at a guard beyond reach is charged and never goes negative; walls, routes, a blade across a wall and diagonal adjacency decide the estimate, with the seven-tile walk of two 7-AP turns pinned; a closing guard opens the fight at exactly eight tiles for a blade; AP is live across a mid-turn break, real-time actions cost while alerted, a guard-phase break gives a fresh turn, an all-clear frees real time; an alert guard reaching its fix stands down and travel reopens; a void or off-map fix goes to the nearest ground; pending casualties keep turns and their timers; every investigating guard moves each tick, the headless player gives them the tick and a whole run through the real-time approach terminates; a tick on the 36-guard playtest map stays inside the frame budget). `tests/tactics-sight.test.mjs` and `tests/tactics.test.mjs` updated where they asserted the old any-alert-is-contact rule; fixtures in the retreat, recovery, group, scale, exploration and squad-bot tests that used a far alert guard to hold turn mode now place that guard within two-turn reach (with an empty gun where guard phases run).
