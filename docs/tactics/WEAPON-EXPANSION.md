...
The flamethrower addition supplies 48 more sprites (384 total): all nine base outfits and seven Red Hats outfits in standing, kneeling, and prone poses, with painted projector, hose, and backpack tanks. The foreman shares his existing uniform across factions. Flamethrower prompts, reference images, and built-in ImageGen output paths are recorded in `art/flamethrower-sources.json` and `art/red-hats-flamethrower-sources.json`, and included in the main source list. Filter the gallery with `?weapon=flamethrower` to review the set. Generate Red Hats contact sheets with `./tools/review-flamethrowers.ps1 -Outfit red-hats`. Firing is animated in code from each sprite's measured nozzle; see `art/flamethrower-animation.md` and preview it in `dist/tactics/flame-art.html`.
