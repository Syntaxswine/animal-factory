...
- The game opens with $20,000 cash, intended to recruit a squad and pay for roughly one to two weeks of service (user clarification, 2026-09-19). This is the opening squad budget; tune recruitment and contract costs against that target. A small cash bonus is paid when the tutorial is finished. The tutorial does not exist yet; it is expected to take about a week of in-game time.
...
- After the initial town, the player is pointed to three other nearby factories that pay more. Their income supports the transition from the opening squad budget to a sustained campaign.
...
**Against the tree** (1c14d9b). The treasury starts at $0; a liberated factory pays $100 an hour, $2,400 a day, forever; the overmap is three tiles (the starting factory, the freight yard, the outer factory) with income scaling by distance from the start. Contracts are prepaid and nothing drains the treasury on the clock: `advanceTime` in `world.js` is the one place income lands, and the doc above holds that up as a principle, so upkeep belongs in the same calculation. There is no town location type (`factory` and `yard` only), no purchase or investment on a location, no way to lose a won map (liberation is permanent), no weapon condition or repair anywhere (REVIEW.md's open question 5, never built), no tutorial, no social services and no welfare ledger. The hiring ladder currently runs $100 to $10,000 a day with comrade-grade help at $630–790 before compatibility surcharges and term discounts. These implementation numbers must be checked against the clarified target: $20,000 recruits and funds a squad for roughly one to two weeks. The earlier characterization as a war chest rather than a squad-pay budget was incorrect. Squad size, intended starting quality and the role of the four existing comrades still need definition; the target does not guarantee that an all-elite squad is affordable.
