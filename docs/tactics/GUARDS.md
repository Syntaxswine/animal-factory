...
- **The shout.** An Alert guard shouts once, on entering the state, whatever put it there (a sighting, a gunshot alarm, a hit, a colleague's shout). The shout is its alert bark, in register. Every colleague within its archetype's radius (table below; base 12, Rebel 0) that is not already Alert or Broken hears it: one that heeds it takes the shouter's fix and goes Alert, and shouts in turn, so an alarm runs down a chain of obedient guards (bounded by their radii, never repeating: a guard already Alert keeps its own, better fix); one that does not goes Suspicious toward the shouter's own tile. A shout without a fix (a guard hit by a colleague's stray, a guard catching fire) rallies the others on the shouter. A Jester's listeners only ever go Suspicious: the joke is not believed. The squad hears the shout and the answer count when the shouter is within thirty tiles or in sight ("1 guard answers Boris's shout.").
...
- **Grief.** A guard whose trusted or bonded colleague falls within thirty tiles takes 15 or 25 stress and remembers it; when its stress passes its nerve trait it breaks on the spot, toward the killer if the killer is not a guard (an Innocent at nerve 30 breaks on a bonded death if it already carried ten; a Hero at 85 never does). A colleague's bullet as the cause applies the G5 quantities: the mourner's bond toward the killer drops a further 40 (trusted 20) and its grudge rises by the same. Guards work stress off at the mercs' resting rate on the campaign clock (`settleGuards`).
