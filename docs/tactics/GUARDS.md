...
Integration review: G2, G3 and G4 are implemented and included in the canonical integration. G3's module `dist/tactics/archetypes.js` is imported by the engine, and the bond script prints from it. G5 remains a proposal. See [G3/G4 integration checks](G3-G4-INTEGRATION-REVIEW.md) for the combined validation; the implementation sections below distinguish built behavior from remaining proposals.
...
- **Balance.** The headless bot plays one map and never runs the campaign clock, so the benchmark cannot see the meter; what it can see is the death events' stress on the survivors. Forty seeds (1947–1986), knob on, against the canonical tip 6eec4c6: every total identical (38 / 2, 131 shouts answered, 36 guard-on-guard hits, 213 merc friendly-fire hits and 74 retaliations, 139 guards broken, mean squad HP 178 over 11.6 rounds) and no seed changed its result; knob off, twenty runs hash identically. G5 is balance-neutral where the benchmark looks; its effects live on the clock, in downtime and travel, which only a campaign playtest exercises.
