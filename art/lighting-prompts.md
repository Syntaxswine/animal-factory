...
- Light: `--light=catalogue`, key light at (−1, 2, 3), fill 0.5, key 2.8, gain 1.3, no tone mapping,
  flame meshes unlit, the `iron` material re-tinted to `0x5c5f58` (darker than the `0x65615a`
  cooking pot, so the tripod keeps its order). Calibrated against painted
...
- Registration: `--register`, two `#13241d` pixels at alpha 64 on the anchor row, symmetric about
  the footprint centre. After any repaint, restore them with
  `node tools/bake-scenery.mjs --remark=dist/assets/environment/manifest-lighting.json`. Each asset's `footCentre` and `gameScale` are recorded in
