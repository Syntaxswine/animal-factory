...
**Mercenaries** opens the hiring hall on a cleared map: six candidates a campaign day with a portrait, stats, personality type and who it does and does not work with, kit, and a price by the day, week or month ($100 to $10,000 a day); hires arrive beside the selected comrade, contracts run on the campaign clock, and a merc whose contract is up leaves at the next quiet moment. The roster holds eight; keys 1–9 select in roster order.
...
Two equipped weapons swap for free. Equipping either ready slot from storage costs3AP in combat. Reload costs3AP and transfers finite compatible reserves. The backpack is a4×4grid. Rifles and assault rifles use two adjacent cells; pistols, ammo stacks and utilities one. Two ready slots each hold one rifle-sized weapon outside the backpack. The current catalog supports one weapon of each type. Drop/pick up and adjacent teammate transfers preserve loaded rounds. A guard's body must be searched before its contents show or can be taken (3 AP in combat or while any guard is alert, free in calm real time); what it holds is rolled once when the guard falls, from its own equipment: every gun with the rounds it had loaded and 40–100% of each reserve stack, never anything it did not carry. Nearby loot appears in the backpack panel; golden diamonds mark piles.

Combat pacing: an alerted guard that could bring a squad member under fire within two of its turns (its own movement and weapon range, walls and routes as they are) starts or holds turn mode; alerted guards farther away close in real time and only warn you ("You are pretty sure someone heard that."), then give up and stand down if they find nobody at their fix. In the campaign a guard going alert shouts: colleagues within its archetype's radius that heed it converge on its fix and shout in turn, the rest grow suspicious of the shouter; guards hit by a colleague's bullet hold grudges and can shoot back, and a guard whose friend falls beside it can break. Mercs keep a happiness meter in the campaign: comrades they resent on the same map wear it down five a day, friends and clean fights lift it, a partner's death or capture drops it at once, and a merc a full day at zero quits the squad (skills kept, off the map). While any guard is alert, actions other than walking cost their combat AP even in real time and nothing refills until a guard phase ends; opening fire from real time opens a turn you must end. A fresh fight (no guard alert) starts with a full turn.

Choose Easy or Standard for the next run, then Restart. Easy automatically stabilizes fallen workers; Standard gives six squad-turn ends for medkit treatment. A stabilized worker stands again three full squad turns after the stabilization (Easy: after the downing), at 5 HP with fatigue at maximum; turn mode continues until then, so clearing the map does not shortcut the delay. On whole-squad defeat, stabilized mercs are captured and bleeding mercs die. The run retains a captured roster; the mid-to-late-game rescue facility and campaign continuation are planned, not yet playable. Wire cutters and medical skill retain their existing rules.
