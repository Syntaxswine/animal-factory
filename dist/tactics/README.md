...
Combat pacing: an alerted guard that could bring a squad member under fire within two of its turns (its own movement and weapon range, walls and routes as they are) starts or holds turn mode; alerted guards farther away close in real time and only warn you ("You are pretty sure someone heard that."). In the campaign a guard going alert shouts: colleagues within its archetype's radius that heed it converge on its fix and shout in turn, the rest grow suspicious of the shouter; guards hit by a colleague's bullet hold grudges and can shoot back, and a guard whose friend falls beside it can break., then give up and stand down if they find nobody at their fix. While any guard is alert, actions other than walking cost their combat AP even in real time and nothing refills until a guard phase ends; opening fire from real time opens a turn you must end. A fresh fight (no guard alert) starts with a full turn.
