...
export function createLightHorse(data,texture=null){
 ...
 const rifle=createWorkerRifle(texture);let equipped=rifle;root.add(rifle.root);const palmLocal=new THREE.Vector3(.052,-.010,0),contacts=[];
 ...
 function pose(mode='carry',heading=0){root.rotation.y=0;reset();contacts.length=0;const carry=equipped.carry||{position:[.24,1.005,.035],axis:[.20,.38,-.90],hands:[1,-1]};equipped.root.visible=mode==='carry';if(equipped.mount)equipped.mount.visible=mode==='carry';if(equipped.hose)equipped.hose.visible=mode==='carry';if(mode==='carry'){const axis=V(carry.axis).normalize();equipped.root.quaternion.setFromUnitVectors(V([1,0,0]),axis);equipped.root.position.fromArray(carry.position);root.updateMatrixWorld(true);const handQ=new THREE.Quaternion().setFromUnitVectors(V([0,-1,0]),axis);for(const l of limbs){if(!carry.hands.includes(l.side))continue;const anchor=l.side===1?equipped.anchors.grip:equipped.anchors.support;solveArm(l,anchor.getWorldPosition(new THREE.Vector3()),handQ);}equipped.updateHose?.(root);}
  root.rotation.y=heading*Math.PI/180;root.updateMatrixWorld(true);skeleton.update();return contacts;
 }
 function diagnostics(){root.updateMatrixWorld(true);skeleton.update();const box=new THREE.Box3(),v=new THREE.Vector3();for(const m of parts){const p=m.geometry.attributes.position;for(let i=0;i<p.count;i++){v.fromBufferAttribute(p,i);m.applyBoneTransform(i,v);box.expandByPoint(v.applyMatrix4(m.matrixWorld));}}return {triangles:data.triangles,rifleTriangles:rifle.triangles,weapon:equipped.id||'rifle',weaponTriangles:equipped.triangles,bones:bones.length,skinnedMeshes:parts.length,min:box.min.toArray(),max:box.max.toArray(),contacts:contacts.map(c=>{const l=limbs.find(l=>l.side===c.side),grip=(c.side===1?equipped.anchors.grip:equipped.anchors.support).getWorldPosition(new THREE.Vector3()),palm=bones[l.wr].localToWorld(palmLocal.clone());return {side:c.side,grip:grip.toArray(),palm:palm.toArray(),error:grip.distanceTo(palm)};}),sourceTriangles:data.sourceTriangles};}
 pose('neutral');return {root,parts,bones,skeleton,rifle,get weapon(){return equipped;},equipWeapon,material,grey,pose,diagnostics,setGraphic(value){graphicUniform.value=value?1:0;},setGrey(value){for(const p of parts)p.material=value?grey:material;},dispose(){for(const p of parts)p.geometry.dispose();material.dispose();grey.dispose();rifle.dispose();}};
}
