...
export const passable=(m,p)=>['floor','yard'].includes(terrainAt(m,p.x,p.y,levelOf(p)));
...
export function blockedEdge(m,a,b){const k=edgeBetween(a,b);return !k||m.edges?.[k]==='wall';}
...
export function stairSet(m){return new Set((m.stairs||[]).map(p=>stairKey(p.x,p.y,p.z)));}
export function neighbors(m,p,stairs=stairSet(m)){
 ...
 for(const dz of [-1,1])if(stairs.has(stairKey(p.x,p.y,Math.min(z,z+dz)))&&passable(m,{x:p.x,y:p.y,z:z+dz}))out.push({x:p.x,y:p.y,z:z+dz,cost:2});return out;
}
...
export function blankMap(name='Untitled local map'){
 return {version:2,width:W,height:H,levels:LEVELS,name,terrain:Array.from({length:H},()=>Array(W).fill('yard')),upper:[{},{}],edges:{},stairs:[],starts:[{x:3,y:4,z:0},{x:3,y:6,z:0},{x:2,y:5,z:0},{x:2,y:7,z:0}],guards:[],exits:[{x:3,y:5,z:0}]};
}
...
export function addStairs(m,x,y,z){if(!inBounds(x,y,z)||z>=LEVELS-1)return false;setTerrain(m,x,y,z,'floor');setTerrain(m,x,y,z+1,'floor');if(!m.stairs.some(p=>p.x===x&&p.y===y&&p.z===z))m.stairs.push({x,y,z});return true;}
export function factoryMap(){
 ...
 addStairs(m,3,10,0);addStairs(m,3,12,1);return m;
}
...
export function validateMap(raw,{connectivity=true}={}){
 ...
 if(!Array.isArray(raw.terrain)||raw.terrain.length!==H||raw.terrain.some(r=>!Array.isArray(r)||r.length!==W||r.some(t=>!['yard','floor','crate','void'].includes(t))))return [...errors,'Invalid ground terrain.'];
 if(!Array.isArray(raw.upper)||raw.upper.length!==2)return [...errors,'Expected two sparse upper levels.'];
 for(const layer of raw.upper){if(!layer||typeof layer!=='object'||Array.isArray(layer))return [...errors,'Invalid upper floor.'];for(const [k,v]of Object.entries(layer)){const [x,y]=k.split(',').map(Number);if(k!==tileKey(x,y)||!inBounds(x,y)||!['yard','floor','crate'].includes(v))return [...errors,'Invalid upper floor tile.'];}}
 if(!raw.edges||typeof raw.edges!=='object'||Array.isArray(raw.edges))return [...errors,'Missing edge barriers.'];
 for(const [k,v]of Object.entries(raw.edges)){const p=/^(e|s):(-?\d+):(-?\d+)(?::([12]))?$/.exec(k);if(!p)return [...errors,'Invalid edge key.'];const x=Number(p[2]),y=Number(p[3]),z=Number(p[4]||0);if(!['wall','door'].includes(v)||k!==edgeKey(p[1],x,y,z)||(p[1]==='e'?(x< -1||x>=W||y<0||y>=H):(x<0||x>=W||y< -1||y>=H)))return [...errors,'Invalid edge location or type.'];}
 ...
 if(!Array.isArray(raw.stairs)||raw.stairs.length>4096||raw.stairs.some(p=>!point(p)||!Number.isInteger(p.z)||p.z>=2))return [...errors,'Invalid stairs: lower level must be 0 or 1.'];
 ...
 if(!passable(raw,raw.exits[0]))errors.push('Travel marker needs a walkable floor.');if(errors.length||!connectivity)return errors;
...
export function generateMap(seed=7,name='Generated test',population=MAX_GUARDS){
