import {factoryMap,generateMap,blockedEdge,tileKey} from './maps.js';
...
export function travelReason(world,destination) {
  ...
  if(squad(s).some(u=>Math.hypot(u.x-exit.x,u.y-exit.y)>2))return 'Gather every living squad member within 2 tiles of the travel marker.';
  return '';
}
function landing(s,start,occupied) {
  const q=[start],seen=new Set([tileKey(start.x,start.y)]);
  for(let i=0;i<q.length;i++){const p=q[i];if(walkable(s,p.x,p.y)&&!occupied.has(tileKey(p.x,p.y)))return p;
    for(const b of [{x:p.x+1,y:p.y},{x:p.x-1,y:p.y},{x:p.x,y:p.y+1},{x:p.x,y:p.y-1}]){const k=tileKey(b.x,b.y);if(walkable(s,b.x,b.y)&&!seen.has(k)&&!blockedEdge(s,p,b)){seen.add(k);q.push(b);}}
  }return null;
}
export function travel(world,destination) {
  const error=travelReason(world,destination);if(error)return {ok:false,error};
  const previous=currentMap(world),next=world.states[destination]||createGame(1947,world.definitions[destination]);
  const incoming=structuredClone(previous.units.filter(u=>u.team==='squad'));
  const occupied=new Set(guards(next).map(u=>tileKey(u.x,u.y)));
  for(const u of incoming){const p=landing(next,next.definition.starts[u.id],occupied);if(!p)return {ok:false,error:'No free arrival tile.'};u.x=p.x;u.y=p.y;u.alert=false;u.lastKnown=null;if(alive(u))occupied.add(tileKey(p.x,p.y));}
