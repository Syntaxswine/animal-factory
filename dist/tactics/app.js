import {touchCamera} from './touch-camera.js';
import {overwatchTiles} from './overwatch-view.js';
import {personalityDescription} from './personalities.js';
import {quitHoursLeft,onContract} from './happiness.js';
import {compatibility,describeKit,previewRecruit,portraitCrop,contractMinutesLeft,contractPrices,describeArchetypeShort,PITCH,TERMS,ROSTER_MAX} from './recruits.js';
import {drawFlamethrower} from './flamethrower-art.js';
import {effectMs,effectClock,drawFlameBurst,nozzleOffset,torsoLift,missLanding,shooterShake,targetFlinch,holdsFallenSprite,TUFTS} from './flame-effect.js';
...
import {paintDaylight,daylightStrength} from './daylight.js';
import {sightScale,stateMinutes} from './light-sources.js';
import {runScenePass} from './scene-passes.js';
import {installGroundCover} from './ground-cover.js';
import {installLightPools} from './light-render.js';
...
import {W,H,WEAPONS,overwatchRange,headingTo,arrangeInventory,stowWeapon,equipCutters,allocateSkill,setSneaking,setOverwatch,stepInvestigation,inventoryTransfer,turnTo,AIM_ZONES,zoneVisible,canSee,moveGroup,key,alive,incapacitated,medicalCost,recoveryEnds,stabilizePreview,stabilize,searchPreview,searchBody,pileContents,cutPreview,cutFence,squad,guards,occupant,tile,walkable,createGame,groundTarget,attackGround,STANCES,stanceOf,setStance,movementNeighbors,navigationPath,pathCost,pathTo,move,stepMovement,previewAttack,attack,equip,reload,endTurn,stepEnemy,canControl,sightOf,identifyRange,detectRange,stateOf,unit,roster,combatCosts} from './engine.js';
// G2 guard states shown over a detected guard (Rest and Alert need no tag: the sprite and the contact marker say it).
const STATE_TAGS={suspicious:'?',searching:'SEARCHING',standdown:'STANDING DOWN',broken:'BROKEN'};
const $=id=>document.getElementById(id),canvas=$('map'),mainCtx=canvas.getContext('2d'),mini=$('mini').getContext('2d');
let mapConfirmation=null;
...
if(!customMap){try{const response=await fetch(new URL('./default-factory.json',import.meta.url),{cache:'no-store'});if(!response.ok)throw Error('HTTP '+response.status);customMap=parseMap(await response.text());}catch(e){const notice=document.createElement('p');notice.textContent='Your factory map could not load. Reload this page to retry. '+e.message;notice.setAttribute('role','alert');document.body.replaceChildren(notice);throw e;}}
...
installLightPools((w,h)=>Object.assign(document.createElement('canvas'),{width:w,height:h}));
...
function drawObjects(now){const numbers=new Map(cards().map((u,i)=>[u.id,i+1]));for(const p of Object.values(s.glimpses||{}))if(levelOf(p)===renderLevel)diamond(p.x,p.y,'#e8cb6930','#e8cb69c0',0,.5);for(const [id,p] of Object.entries(s.contacts||{}))if(!s.detected.has(Number(id))&&levelOf(p)===renderLevel){diamond(p.x,p.y,null,'#b4937260',0,.65);textLabel('?',p.x,p.y,'#b49372');}
 ...
 for(const pile of s.loot||[])if(pileContents(pile).length&&levelOf(pile)===renderLevel&&inView(pile,b)&&s.visible.has(key(pile.x,pile.y,renderLevel)))objects.push({...pile,type:'loot'});
 for(const u of s.units)if(u.casualty!=='captured'&&u.casualty!=='quit'&&!u.away&&levelOf(u)===renderLevel&&inView(u,b)&&(u.team==='squad'||s.detected.has(u.id)||fallenVisible(s,u,renderLevel)))objects.push({...u,type:'actor',unit:u});
 // A burning tile is a scorch under everything standing on it plus clumps of flame sorted like any other sprite.
 for(const fire of s.fires||[])if(levelOf(fire)===renderLevel&&inView(fire,b)&&s.seen.has(key(fire.x,fire.y,fire.z))){const visible=s.visible.has(key(fire.x,fire.y,fire.z));objects.push({x:fire.x,y:fire.y,type:'scorch',fire,visible});for(const clump of fireClumps(fire))objects.push({...clump,type:'fire',fire,visible});}
 ...
 for(const obj of objects){const {x,y,type}=obj,visible=obj.visible??s.visible.has(key(x,y,renderLevel));ctx.globalAlpha=visible?1:.45;
  if(type==='prop'){if(PROPS[obj.kind]?.seeThrough)ctx.globalAlpha*=seeThroughAlpha(propScreenBox(project,camera.zoom,obj),propCells(obj),looking);art.prop(ctx,project,camera.zoom,obj);}
  ...
  else if(type==='scorch'||type==='fire'){const p=project(x,y);fireArt.draw(ctx,type==='fire'?'clump':'scorch',p.x,p.y,{zoom:camera.zoom,pixelRatio:mainCtx.canvas.width/width,turns:obj.fire.turns,variant:obj.variant,frame:reducedMotion.matches?0:fireFrame(now,obj.phase),grow:(obj.size||1)*fireGrowth(obj.fire,now)});}
   ...
   if(incapacitated(u)){drawBody(ctx,load,u,p,camera.zoom);diamond(x,y,'#bc8f3a77','#edc87a');textLabel(u.casualty==='stable'?'STABLE':u.name+' · '+u.bleedTurns+' turns',x,y,'#ffd98b',11);ctx.globalAlpha=1;continue;}if(!alive(u)&&!flameHolds(u)){drawBody(ctx,load,u,p,camera.zoom);drawDeathDrops(ctx,load,u,p,camera.zoom);ctx.font=`${10*camera.zoom}px monospace`;ctx.textAlign='center';ctx.fillStyle='#b5a17c';ctx.fillText('×',p.x-17*camera.zoom,p.y+7*camera.zoom);ctx.globalAlpha=1;continue;}
   ...
   const nudge=flamePose(u);ctx.save();ctx.translate(p.x+nudge.x,p.y+3*camera.zoom+nudge.y);ctx.scale(u.facing,1);const exposed=s.exposed?.[u.id];if(u.team==='guard'&&exposed){ctx.beginPath();const height=frame.contentHeight/4*camera.zoom;for(const [zone,lo,hi]of [['legs',0,.38],['torso',.38,.85],['head',.85,1]])if(exposed.includes(zone)||zone==='torso'&&exposed.includes('weapon'))ctx.rect(-sw/2,-3*camera.zoom-height*hi,sw,height*(hi-lo));ctx.clip();}if(img.complete&&img.naturalWidth)ctx.drawImage(img,-sw/2,-sh,sw,sh);else{ctx.fillStyle=color;ctx.fillRect(-sw/4,-sh,sw/2,sh);}drawFlamethrower(ctx,u,camera.zoom);ctx.restore();
   ...
   ctx.font=`bold ${9*camera.zoom}px monospace`;ctx.textAlign='center';ctx.fillStyle=color;ctx.fillText((u.team==='squad'?`${numbers.get(u.id)??''} ${u.name}`:u.name)+(u.burningTurns?' [FIRE '+u.burningTurns+']':'')+(stanceOf(u)==='standing'?'':stanceOf(u)==='kneeling'?' [K]':' [P]'),p.x,p.y+17*camera.zoom);
   if(u.team==='guard'&&s.detected.has(u.id)&&STATE_TAGS[stateOf(u)]){ctx.font=`bold ${9*camera.zoom}px monospace`;ctx.textAlign='center';ctx.fillStyle='#e8d9ad';ctx.fillText(STATE_TAGS[stateOf(u)],p.x,p.y-(frame.contentHeight/4+13)*camera.zoom);}
   if(renderLevel===viewLevel&&alive(u))sprites.push({id:u.id,x:p.x-sw/2,y:p.y-frame.contentHeight/4*camera.zoom,w:sw,h:(frame.contentHeight/4+10)*camera.zoom});
  }ctx.globalAlpha=1;
 }
 ctx.globalAlpha=1;
}
// A new burning tile pops up after its ripple delay; under reduced motion it is simply there.
const fireGrowth=(fire,now)=>reducedMotion.matches?1:igniteGrowth(fire,now-(fireBorn.get(key(fire.x,fire.y,fire.z))??now));
// Birth times are noted for every fire, on screen or not, so scrolling a blaze into view does not replay its ignition.
function trackFires(now){const live=new Set();for(const fire of s.fires||[]){const id=key(fire.x,fire.y,fire.z);live.add(id);if(!fireBorn.has(id))fireBorn.set(id,now);}for(const id of fireBorn.keys())if(!live.has(id))fireBorn.delete(id);}
// Attack effects play back to back from the moment they appear; a flamethrower burst also moves the sprites it involves.
function trackEffect(now){
 if(s.effect!==lastEffect){lastEffect=s.effect;effectStart=now;effectUntil=now+Math.max(350,effectMs(s.effect));}
 const clock=s.effect&&now<effectUntil?effectClock(s.effect,now-effectStart):null;
 flame=clock?.event?.incendiary?{event:clock.event,t:clock.local,still:reducedMotion.matches}:null;
}
const flameOutcome=e=>!e.hit?'miss':e.downed?.includes(e.target)?'kill':'hit';
function flamePose(u){
 if(!flame||flame.still)return {x:0,y:0};
 if(u.id===flame.event.shooter)return shooterShake(flame.t,camera.zoom,u.facing);
 return u.id===flame.event.target?targetFlinch(flame.t,camera.zoom,flameOutcome(flame.event)):{x:0,y:0};
}
// A body the flame burns down stays standing until the jet reaches it and the soot hides the swap.
const flameHolds=u=>!!flame&&!flame.still&&u.id===flame.event.target&&flame.event.downed?.includes(u.id)&&holdsFallenSprite(flame.t);
function drawFlame({event:e,t,still}){
 const shooter=unit(s,e.shooter),target=unit(s,e.target);if(!shooter||!target)return;
 const nozzle=nozzleOffset(shooter,unitArt(shooter),camera.zoom),base=project(e.ax,e.ay),shake=still?{x:0,y:0}:shooterShake(t,camera.zoom,shooter.facing),ground=project(e.bx,e.by),outcome=flameOutcome(e),seed=Math.floor(effectStart);
 const landing=outcome==='miss'?missLanding(e,seed):null,to=landing?project(landing.x,landing.y):{x:ground.x,y:ground.y-torsoLift(target,camera.zoom)};
 drawFlameBurst(ctx,{from:{x:base.x+nozzle.x+shake.x,y:base.y+nozzle.y+shake.y},dir:{x:nozzle.dx,y:nozzle.dy},to,ground,t,zoom:camera.zoom,outcome,seed,tuft:TUFTS[target.species],still});
}
function drawPreview(){
 if(hover&&canControl(s,selected())&&!s.queue.length){const u=hoverActor!==null?unit(s,hoverActor):null;
...
// What a registered scene pass is given; see scene-passes.js.
const sceneView=level=>({project,zoom:camera.zoom,level,bounds:bounds(camera,width,height),state:s,minutes:world.clock.minutes});
function draw(now){if(!hover||drag||camera.zoom<.2)setCursorKind(drag?'pan':camera.zoom<.2?'overview':'default');ctx.clearRect(0,0,width,height);ctx.fillStyle='#202c29';ctx.fillRect(0,0,width,height);if(camera.zoom<.2){sprites.length=0;compositeLayers(mainCtx,viewLevel,width,height,(ink,level)=>{sectorOverview(ink,project,s,level,s.seen,level===0);for(const u of s.units)if(levelOf(u)===level&&alive(u)&&(u.team==='squad'||s.detected.has(u.id))){const p=project(u.x,u.y);ink.fillStyle=u.team==='squad'?'#ffda79':'#ff9275';ink.fillRect(p.x-2,p.y-2,4,4);}});return;}sprites.length=0;trackEffect(now);trackFires(now);compositeLayers(mainCtx,viewLevel,width,height,(ink,level)=>{ctx=ink;renderLevel=level;try{drawTerrain();runScenePass('dressing',ctx,sceneView(level)); if((watchAim||watchPreview||selected().overwatch)&&renderLevel===levelOf(selected())){const u=selected(),a=watchAim&&hover?{...u,heading:headingTo(u,hover)}:u;for(const q of overwatchTiles(s,a,overwatchRange(u)))diamond(q.x,q.y,q.clear?"#e8cb6970":"#74653595");const r=a.heading*Math.PI/180,p=project(a.x,a.y),q=project(a.x+Math.cos(r)*overwatchRange(u),a.y+Math.sin(r)*overwatchRange(u));ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(q.x,q.y);ctx.strokeStyle="#ffe597";ctx.lineWidth=2;ctx.stroke();} if(!watchAim&&!watchPreview&&!selected().overwatch&&showCone&&renderLevel===levelOf(selected())){const u=selected(),n=Math.max(8,Math.ceil(u.cone/2)),lobe=range=>{const pts=[project(u.x,u.y)];for(let i=0;i<=n;i++){const a=(u.heading-u.cone/2+u.cone*i/n)*Math.PI/180,c=Math.cos(a),k=Math.sin(a),r=range({x:u.x+c,y:u.y+k});pts.push(project(u.x+r*c,u.y+r*k));}return pts;};const reach=60*sightScale(daylightStrength(stateMinutes(s)));poly(lobe(q=>detectRange(u,q,reach)),'#e8cb6910','#e8cb6950');poly(lobe(q=>identifyRange(u,q,reach)),'#e8cb6924','#e8cb69b0');}runScenePass('light',ctx,sceneView(level));drawObjects(now);}finally{ctx=mainCtx;renderLevel=viewLevel;}});ctx=mainCtx;renderLevel=viewLevel;paintDaylight(mainCtx,world.clock.minutes,width,height);const pointerView=[camera.x,camera.y,camera.zoom,viewLevel,width,height,s.revision].join(':');if(cursor&&!drag&&pointerView!==cursorView){cursorView=pointerView;const r=canvas.getBoundingClientRect();hitTest({clientX:r.left+cursor.x,clientY:r.top+cursor.y});updateHover();}drawPreview();drawActionCost();if(terrainAim&&hover){const q=project(hover.x,hover.y);ctx.strokeStyle='#ffac62';ctx.lineWidth=2;ctx.beginPath();ctx.ellipse(q.x,q.y,WEAPONS[selected().weapon].blast*28*camera.zoom,WEAPONS[selected().weapon].blast*14*camera.zoom,0,0,Math.PI*2);ctx.stroke();}
 if(s.effect&&now<effectUntil){
  const e=effectClock(s.effect,now-effectStart)?.event??{},projectShot=p=>project(p.x,p.y,(p.h-viewLevel*3)*24);
  ...
  if(e.incendiary){if(flame&&(e.az===viewLevel||e.bz===viewLevel))drawFlame(flame);}
  else if(e.trajectories?.length){for(const shot of e.trajectories){if(Math.floor(shot.origin.h/3)!==viewLevel&&shot.z!==viewLevel)continue;const a=projectShot(shot.origin),b=projectShot(shot);ctx.strokeStyle=shot.kind==='unit'?'#ffe6a2':'#c2c5a0';ctx.lineWidth=1.5*camera.zoom;ctx.beginPath();ctx.moveTo(a.x,a.y);for(const point of shot.path||[shot]){const q=projectShot(point);ctx.lineTo(q.x,q.y);}ctx.stroke();if(shot.kind!=='range'){ctx.beginPath();ctx.arc(b.x,b.y,3*camera.zoom,0,Math.PI*2);ctx.stroke();}}}
  else if(e.az===viewLevel||e.bz===viewLevel){const a=project(e.ax,e.ay,30),b=project(e.bx,e.by,30);ctx.strokeStyle=e.hit?'#ffe6a2':'#c2c5a0';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();ctx.beginPath();ctx.arc(b.x,b.y,7,0,7);ctx.stroke();}
 ...
 // Registered overlays paint last, over the daylight wash: beams and the like.
 runScenePass('overlay',mainCtx,sceneView(viewLevel));
}
...
function drawActionCost(){if(watchAim)return;
...
function updateHover(){
 ...
 if(watchAim){route=null;hoverAction={kind:'shoot',action:'watch'};setCursorKind('shoot');$('hint').textContent='Click to face this direction · Escape to cancel · Use Overwatch to reserve the shot';return;}const u=selected(),edge=fenceAtCursor(s,u,cursor,project,camera.zoom,viewLevel);hoverAction=contextAction(s,u,{actorId:hoverActor,point:{...hover,z:viewLevel},edge});if(terrainAim&&WEAPONS[u.weapon].blast)hoverAction={kind:'shoot',action:'ground',point:{...hover,z:viewLevel}};setCursorKind(hoverAction.kind);
...
function sync(){ {const settled=resolveRetreat(world);if(settled.ok){adoptMap(settled.state);message('The fight is over for those who stayed. '+squad(s).length+' regrouped in '+s.definition.name+'.');}} syncClock();if(hover)updateHover(); $('sight-info').textContent=`${selected().cone}° field · ${sightOf(selected()).bino}° binocular · 60 tile identify / 75 tile landscape`;$('turn').disabled=!canControl(s,selected())||!!s.queue.length;selectedIds=new Set([...selectedIds].filter(id=>alive(unit(s,id))));selectedIds.add(s.selected);
 ...
 if(!w.blast){terrainAim=false;terrainTarget=null;}$('terrain-aim').hidden=!w.blast;$('terrain-aim').setAttribute('aria-pressed',terrainAim);$('aim-zone').disabled=!!w.blast;for(const option of $('aim-zone').options){const hidden=t&&!t.ground&&!w.blast&&(!canSee(s,u,t)||!zoneVisible(s,u,t,option.value));option.disabled=!!hidden;option.textContent=AIM_ZONES[option.value].label+(hidden?' · hidden':'');}
 if(!t)targetId=null;
 ...
 $('weapon-stats').textContent=`${w.name} · ${w.pellets?w.pellets+' × ':''}${w.damage} damage\n${w.cost} AP · ${w.range} tile ${w.blast&&!w.thrown?'effective range':'range'} · ${w.mag?`${u.ammo[u.weapon]}/${w.mag} ${u.weapon==='flamethrower'?'fuel bursts':'rounds'} · ${reserve(u,u.weapon)} reserve`:'melee'}`+(u.weapon==='flamethrower'?' · 180 damage within 3 tiles, falling to 45 at 10 · Tank explosion: torso hit 25%, weapon hit 90% · Fatal within 1 tile; fire within 5 tiles; panic for 3 turns':'');
 ...
 $('burst').disabled=!control||!w.burstRounds;$('burst').textContent=`Burst: ${burst?'on':'off'} [B]`;$('burst').setAttribute('aria-pressed',burst);
 ...
 $('target-info').innerHTML=t?`<b>${t.name}</b> / L${levelOf(t)+1} / ${t.ground?'Blast target':WEAPONS[t.weapon].short}<br>${p.ok?`<strong>${p.chance}%</strong> aim · ${p.cost} AP${shotObstruction(p)} · ${p.pellets>1?p.pellets+' × ':p.rounds>1?p.rounds+' × ':''}${p.damage} damage${p.beyond?' · HEAVY SCATTER':''}${p.tankChance?' · '+Math.round(p.tankChance*100)+'% tank explosion on hit':''}${p.coverPenalty?` · ${p.heightCover&&!p.cover?'HEIGHT COVER':'COVER'} −${p.coverPenalty}%`:''}${p.rangePenalty?` · UPHILL +${p.rangePenalty} distance`:''}`:`${p.reason}${p.cover?' · in cover':''}`}`:'Select a visible guard to inspect a shot.';
 $('attack').disabled=!control||!p?.ok;$('attack').textContent=burst&&w.burstRounds?`Fire ${w.burstRounds}-round burst [F]`:w.mag?'Fire weapon [F]':'Melee attack [F]';
 $('end').disabled=s.phase!=='player'||s.queue.length>0;
 $('squad').innerHTML=cards().map((u,i)=>`<button class="squad-card" data-unit="${u.id}" aria-pressed="${selectedIds.has(u.id)}" ${!alive(u)?'disabled':''}><span class="num">${String(i+1).padStart(2,'0')}</span><img alt="" src="${unitArt(u).src}"><div class="info"><strong>${u.name}</strong><small>${alive(u)?`L${levelOf(u)+1} · ${STANCES[stanceOf(u)].label} · ${WEAPONS[u.weapon].short} · ${u.hp} HP${u.burningTurns?' · FIRE '+u.burningTurns+' turns':''}${quitHoursLeft(u,world.clock.minutes)!==null?' · QUITS IN '+Math.ceil(quitHoursLeft(u,world.clock.minutes))+' h':''}${u.contract&&!u.contract.expired?' · CONTRACT '+hoursLabel(contractMinutesLeft(u,world.clock.minutes)):u.contract?.expired?' · CONTRACT UP':''}`:incapacitated(u)?(u.casualty==='stable'?`STABILIZED · up after ${recoveryEnds(s,u)} turn end${recoveryEnds(s,u)===1?'':'s'}`:`BLEEDING · ${u.bleedTurns} turns`):u.casualty==='captured'?'CAPTURED':u.casualty==='quit'?(u.contract?.ended==='contract'?'CONTRACT ENDED':u.contract?.ended==='released'?'PAID OFF':'QUIT'):u.away?'CROSSED · '+world.definitions[u.away.destination].name:'DEAD'}</small><div class="bar"><i style="width:${u.hp/u.maxHp*100}%"></i></div><div class="bar ap"><i style="width:${u.ap/u.maxAp*100}%"></i></div><small>${['explore','won'].includes(s.phase)?'READY':`${u.ap} / ${u.maxAp} AP`}</small></div></button>`).join('');
 ...
 $('group-status').textContent=selectedIds.size+' selected · Primary: '+selected().name;$('sneak').textContent=u.sneaking?'Sneaking · on':'Sneaking · off';$('watch-aim').disabled=!control||!!u.overwatch;$('watch-aim').setAttribute('aria-pressed',watchAim);$('watch-range').max=w.range;$('watch-range').value=overwatchRange(u);$('watch-range').disabled=!control||!!u.overwatch;$('watch-distance').textContent=overwatchRange(u)+' tiles';$('watch').textContent=u.overwatch?'Overwatch reserved · '+overwatchRange(u)+' tiles':'Overwatch · '+w.cost+' AP';$('watch').disabled=!control||s.phase!=='player'||!!u.overwatch||!w.mag||u.ap<w.cost||u.ammo[u.weapon]<1;renderUtilities();renderInventory();renderSheet();drawMinimap();if($('hiring-screen').open&&hallKey()!==hallShown)renderHiring();lastRevision=s.revision;if(performance.now()>toastUntil){if(s.phase==='enemy')$('hint').textContent='Guard turn · Your squad will regain AP when the guards finish.';else if(s.queue.length)$('hint').textContent='Moving · Escape to stop';else if(over)$('hint').textContent='Operation complete · Restart to play again';else if(hover)updateHover();else $('hint').textContent=s.phase==='player'&&!guards(s).some(g=>g.alert)&&s.units.some(p=>p.team==='squad'&&incapacitated(p))?'Squad turn · A comrade is down: turns continue until everyone is on their feet.':s.phase==='player'?'Squad turn · Use every worker before ending the turn.':s.phase==='won'?'Local map cleared · Gather at the blue travel marker, then open Overmap.':'Click ground to explore · Blue marker: gather within 2 tiles to travel · Map border: cross on foot.';}
}
function select(id,toggle=false){mapConfirmation=null;watchAim=false;watchPreview=false;watchPreview=false;terrainAim=false;terrainTarget=null;const picked=unit(s,id);if(!picked||!alive(picked))return;if(toggle){if(selectedIds.has(id)&&selectedIds.size>1){selectedIds.delete(id);if(s.selected===id)s.selected=[...selectedIds][0];}else{selectedIds.add(id);s.selected=id;}}else{selectedIds=new Set([id]);s.selected=id;}hover=null;hoverActor=null;route=null;viewLevel=levelOf(selected());$('level').value=viewLevel;targetId=null;aimZone='torso';$('aim-zone').value=aimZone;shotConfirmation=null;burst=false;s.queue=[];sync();}
...
function chooseEnemy(id){terrainAim=false;terrainTarget=null;const t=unit(s,id);if(!t||!alive(t)||!s.detected.has(id))return;if(targetId===id&&shotConfirmation===shotKey(t)){tryAttack();return;}targetId=id;shotConfirmation=shotKey(t);sync();updateHover();}
...
$('attack').onclick=tryAttack;$('reload').onclick=()=>{reload(s,selected());sync();};$('end').onclick=()=>{endTurn(s);sync();};$('burst').onclick=()=>{if(WEAPONS[selected().weapon].burstRounds){burst=!burst;sync();}};
...
function mapPress(e){canvas.focus();if(e.button===2||e.button===1||e.altKey){drag={x:e.clientX,y:e.clientY};setCursorKind('pan');canvas.setPointerCapture(e.pointerId);return;}if(e.button!==0)return;if(camera.zoom<.2){message('Double-click a sector to inspect it.');return;}hitTest(e);updateHover();
 if(watchAim&&hover){turnTo(s,selected(),headingTo(selected(),hover));watchAim=false;shotConfirmation=null;sync();return;}const action=hoverAction;if(!action)return;
 if(['move','cut','stabilize','travel'].includes(action.action)){
  const confirmation=JSON.stringify([s.revision,s.selected,viewLevel,action,hover?.x,hover?.y]);
  if(mapConfirmation!==confirmation){mapConfirmation=confirmation;message('Click or tap again to confirm '+({move:'movement',cut:'cutting the fence',stabilize:'treatment',travel:'travel'})[action.action]+'.');return;}
  mapConfirmation=null;
 }else mapConfirmation=null;
 ...
 if(action.action==='search'){searchBody(s,selected(),s.loot.find(p=>p.body===action.id));sync();return;}
...
canvas.addEventListener('pointerdown',mapPress);
touchCamera(canvas,camera,{onTap:mapPress,onChange:()=>{mapConfirmation=null;drag=null;cursor=null;hover=null;hoverActor=null;route=null;hoverAction=null;shotConfirmation=null;setCursorKind('pan');}});
...
document.addEventListener('keydown',e=>{if(document.querySelector('dialog[open]'))return;if(e.ctrlKey||e.metaKey||e.altKey)return;if(['INPUT','SELECT','TEXTAREA'].includes(e.target.tagName))return;const k=e.key.toLowerCase();if('123456789'.includes(k)&&k.length===1){const pick=cards()[Number(k)-1];if(pick)select(pick.id,e.shiftKey);}else if(k===' '){e.preventDefault();endTurn(s);sync();}else if(k==='i')showInventory();else if(k==='r'){reload(s,selected());sync();}else if(k==='f')tryAttack();else if(k==='b'&&WEAPONS[selected().weapon].burstRounds){burst=!burst;sync();}else if(k==='c')center();else if(k==='g')$('grid').click();else if(k==='?'||k==='h')$('manual').showModal();else if(k==='escape'){mapConfirmation=null;watchAim=false;s.queue=[];terrainAim=false;terrainTarget=null;shotConfirmation=null;sync();}else if(k==='+'||k==='=')zoom(1.2);else if(k==='-')zoom(1/1.2);else if(k.startsWith('arrow')){e.preventDefault();if(k==='arrowleft')camera.x+=50;if(k==='arrowright')camera.x-=50;if(k==='arrowup')camera.y+=50;if(k==='arrowdown')camera.y-=50;}});
...
function syncClock(){const clock=clockLabel(world)+' · '+worldPhaseLabel(world);if($('campaign-clock').textContent!==clock)$('campaign-clock').textContent=clock;const summary=`${clock} · Treasury $${world.money.toLocaleString()} · +$${incomePerHour(world).toLocaleString()} / hour`;if($('economy-summary').textContent!==summary)$('economy-summary').textContent=summary;}
...
function showOvermap(){
 ...
 for(const [id,definition]of Object.entries(world.definitions)){
  ...
  card.onclick=()=>{const result=travel(world,id);if(!result.ok){$('travel-info').textContent=result.error;return;}adoptMap(result.state);$('overmap').close();sync();};container.append(card);
 }
 const activeMap=currentMap(world),exit=activeMap.definition.exits[0],waiting=away(activeMap);$('travel-info').textContent=`Travel marker: ${exit.x}, ${exit.y}, level ${levelOf(exit)+1}. Gather every living member within 2 tiles. Travel takes 1 hour and is unavailable during combat. Retreat on foot instead: a member standing on the ground within ${BORDER} tiles of a map edge can cross to the map beyond it, even in combat (Cross-edge button under Equipment).`+(waiting.length?` ${waiting.length} waiting beyond the ${waiting[0].away.side} edge in ${world.definitions[waiting[0].away.destination].name}.`:'');
...
function renderUtilities(){const u=selected();$('medical-info').textContent='Medical '+u.medical+'/100 · Stabilize '+medicalCost(u)+' AP · Medkits '+u.medkits+' · Cutters '+(u.slots.includes('wireCutters')?'held':'in backpack — equip to cut')+' · Cut fence 4 AP';const host=$('utility-actions');host.replaceChildren();for(const patient of s.units.filter(p=>p.casualty==='bleeding')){const check=stabilizePreview(s,u,patient),button=document.createElement('button');button.textContent='Stabilize '+patient.name+' · '+patient.bleedTurns+' turns · '+check.cost+' AP';button.title=check.reason;button.disabled=!check.ok;button.onclick=()=>{if(stabilize(s,selected(),patient))sync();};host.append(button);}for(const [edge,kind]of Object.entries(s.edges)){if(kind!=='fence-chainlink'||!edgeCells(edge).some(p=>p.x===u.x&&p.y===u.y&&levelOf(p)===levelOf(u)))continue;const check=cutPreview(s,u,edge),button=document.createElement('button'),other=edgeCells(edge).find(p=>p.x!==u.x||p.y!==u.y);button.style.cursor=check.ok?cursorStyle('cut'):'not-allowed';button.textContent='Cut fence '+(other.x>u.x?'east':other.x<u.x?'west':other.y>u.y?'south':'north')+' · 4 AP';button.title=check.reason;button.disabled=!check.ok;button.onclick=()=>{if(cutFence(s,selected(),edge))sync();};host.append(button);}for(const side of borderSides(u)){const dest=beyond(world,world.current,side);if(!dest)continue;const reason=leaveReason(world,u,side),button=document.createElement('button');button.textContent='Cross the '+side+' edge → '+world.definitions[dest].name+(s.phase==='player'?' · '+crossingCost(s,u)+' AP':' · Free');button.title=reason||'Leave this map on foot. Comrades who stay keep fighting; the squad regroups beyond the edge when the last one crosses or falls. Downed comrades left behind are captured or die.';button.disabled=!!reason;button.onclick=()=>{const name=selected().name,r=leave(world,selected(),side);if(!r.ok){message(r.error);return;}if(r.arrived){adoptMap(r.state);message('The squad regrouped in '+s.definition.name+(r.left.length?' · left behind: '+r.left.map(u=>u.name).join(', '):'')+'.');}else message(name+' crossed the '+side+' edge · '+squad(s).length+' still on this map.');sync();};host.append(button);}const waiting=away(s);if(waiting.length){const note=document.createElement('span');note.textContent=waiting.length+' waiting beyond the '+waiting[0].away.side+' edge in '+world.definitions[waiting[0].away.destination].name+'.';host.append(note);for(const w of waiting){const reason=recallReason(world,w),button=document.createElement('button');button.textContent='Return '+w.name+' across the '+w.away.side+' edge'+(s.phase==='player'?' · '+crossingCost(s,w)+' AP':' · Free');button.title=reason||'Walk back onto the tile it left from.';button.disabled=!!reason;button.onclick=()=>{const side=w.away.side,r=recall(world,w);if(!r.ok){message(r.error);return;}message(w.name+' came back across the '+side+' edge.');sync();};host.append(button);}}if(!host.children.length)host.textContent='Stand beside a wire fence or bleeding teammate to use equipment, or on the ground within '+BORDER+' tiles of a map edge to cross to the next map.';}
...
// The desk's cards: the squad in roster order, without contractors who have gone (a comrade that quit stays, as G5 shows it). Keys 1-9 follow this order.
function cards(){return roster(s).filter(u=>!(u.casualty==='quit'&&u.hired));}
// The open hall is rebuilt only when something it shows has changed, so a rebuild cannot land between a press and its click.
let hallShown='';function hallKey(){return [Math.floor(world.money),hiringDay(world),s.phase,combatCosts(s)?1:0,world.hired.length,...roster(s).map(u=>u.id+':'+(u.casualty||'')+':'+(u.away?'a':'')+':'+(u.contract?u.contract.until+':'+(u.contract.expired?1:0)+':'+u.contract.renewals:''))].join('|');}
function hoursLabel(minutes){if(minutes===null)return '';const h=Math.ceil(minutes/60);return h>=48?Math.floor(h/24)+' d '+(h%24)+' h':h+' h';}
function contractLine(u,now){if(!u.hired)return 'Comrade · no contract · stays for the cause';if(u.casualty==='quit')return 'Gone · '+(u.contract?.ended==='contract'?'contract ended':u.contract?.ended==='released'?'paid off':'quit')+' · paid $'+u.contract.paid.toLocaleString()+' in all';if(['dead','captured'].includes(u.casualty))return u.casualty.toUpperCase()+' · paid $'+u.contract.paid.toLocaleString()+' in all';if(u.contract.expired)return 'Contract up · leaves when the fight ends unless renewed now';return TERMS[u.contract.term].label+' contract · ends in '+hoursLabel(contractMinutesLeft(u,now))+' ('+clockLabel({clock:{minutes:u.contract.until}})+') · $'+u.hired.rate.toLocaleString()+' / day · paid $'+u.contract.paid.toLocaleString()+' in all';}
// The portrait: the head and shoulders of the standing sprite (recruits.js portraitCrop), scaled to the box.
function portrait(u,size=128){const frame=unitArt({...u,stance:'standing'}),c=portraitCrop(frame),k=size/c.w,box=document.createElement('div');box.className='portrait';box.style.width=box.style.height=size+'px';const img=document.createElement('img');img.src=frame.src;img.alt=u.name+' · '+u.species;img.style.width=frame.width*k+'px';img.style.height=frame.height*k+'px';img.style.left=-c.x*k+'px';img.style.top=-c.y*k+'px';box.append(img);return box;}
function renderHiring(){hallShown=hallKey();
 const now=world.clock.minutes,reason=hiringReason(world),money=world.money;
 $('hiring-summary').textContent=clockLabel(world)+' · Treasury $'+money.toLocaleString()+' · +$'+incomePerHour(world).toLocaleString()+' / hour · '+(reason||'Hiring open. The slate turns over at midnight; a hire arrives beside the selected comrade.');
 const host=$('contracts');host.replaceChildren();const gone=[];
 for(const u of roster(s)){if(u.casualty==='quit'&&u.hired){gone.push(u);continue;}if(u.casualty==='quit')continue;
  const card=document.createElement('div');card.className='contract';card.append(portrait(u,64));const info=document.createElement('div');info.className='info';
  const title=document.createElement('strong');title.textContent=u.name+' · '+u.species+(u.archetype?' · '+u.archetype:'')+' · Level '+u.level;info.append(title);
  const line=document.createElement('small');line.textContent=contractLine(u,now);info.append(line);card.append(info);
  if(u.hired&&onContract(u)){const prices=contractPrices(u),actions=document.createElement('div');actions.className='action-row';const why=renewReason(world,u);
   for(const term of Object.keys(TERMS)){const b=document.createElement('button');b.textContent='Renew · '+TERMS[term].label.toLowerCase()+' $'+prices[term].price.toLocaleString();b.disabled=!!why||money<prices[term].price;b.title=why||(money<prices[term].price?'Not enough in the treasury.':prices[term].days+" days' rate; extends from the contract's end");b.onclick=()=>{const r=renew(world,u.id,term);message(r.ok?u.name+' renewed: $'+r.price.toLocaleString()+'.':r.error);sync();renderHiring();};actions.append(b);}
   const whyOff=releaseReason(world,u),off=document.createElement('button');off.textContent='Pay off · no refund';off.disabled=!!whyOff;off.title=whyOff||'Ends the contract now; the merc leaves with what it carries.';off.onclick=()=>{const r=release(world,u.id);message(r.ok?u.name+' was paid off.':r.error);sync();renderHiring();};actions.append(off);card.append(actions);}
  host.append(card);}
 if(gone.length){const p=document.createElement('p');p.className='hiring-note';p.textContent='Gone: '+gone.map(u=>u.name+' ('+(u.contract.ended==='contract'?'contract ended':'paid off')+', paid $'+u.contract.paid.toLocaleString()+' in all)').join('; ')+'.';host.append(p);}
 const list=$('candidates');list.replaceChildren();$('slate-title').textContent='Day '+(hiringDay(world)+1)+' · candidates';const full=contracted(s).length>=ROSTER_MAX;
 for(const c of candidates(world)){
  const card=document.createElement('article');card.className='candidate';const p=previewRecruit(c,WEAPONS),comp=compatibility(c.archetype);
  const head=document.createElement('div');head.className='candidate-head';head.append(portrait(p,128));const id=document.createElement('div');
  const name=document.createElement('strong');name.textContent=c.name;const sub=document.createElement('small');sub.textContent=c.species.toUpperCase()+' · '+c.archetype.toUpperCase()+' · LEVEL '+c.level;const pitch=document.createElement('p');pitch.className='pitch';pitch.textContent='“'+PITCH[c.archetype]+'”';id.append(name,sub,pitch);head.append(id);card.append(head);
  const dl=document.createElement('dl');for(const [k,v] of [['Health',p.maxHp+' HP'],['Action points',p.maxAp+' AP'],['Accuracy',p.accuracy],['Medical',p.medical+' / 100'],['Stealth',p.stealth+' / 100'],['Sight',p.cone+'° · '+sightOf(p).bino+'° binocular']]){const dt=document.createElement('dt');dt.textContent=k;const dd=document.createElement('dd');dd.textContent=v;dl.append(dt,dd);}card.append(dl);
  const lines=[['Training',Object.entries(SKILLS).filter(([id])=>c.skills[id]).map(([id,k])=>k.label+' '+c.skills[id]).join(' · ')||'none yet'],['Species',SPECIES_TRAITS[c.species]?.label||'No species modifiers'],['Personality',c.archetype+': '+describeArchetypeShort(c.archetype)+'.'],['Works well with',comp.works.join(', ')||'nobody in particular'],['Does not work with',comp.clashes.join(', ')||'nobody in particular'],['With this squad',c.fit.rows.map(r=>r.label).join('; ')||'no one to measure against'],['Comes with',describeKit(c,WEAPONS)]];
  for(const [k,v] of lines){const pr=document.createElement('p');const b=document.createElement('b');b.textContent=k+': ';pr.append(b,v);card.append(pr);}
  const price=document.createElement('p');price.className='price';price.textContent='$'+c.rate.toLocaleString()+' / day'+(c.fit.surcharge>1?' (asks more: expects trouble with '+c.fit.trouble.join(' and ')+')':'')+' · week $'+c.prices.week.price.toLocaleString()+' ('+c.prices.week.days+" days' rate) · month $"+c.prices.month.price.toLocaleString()+' ('+c.prices.month.days+" days' rate)";card.append(price);
  const actions=document.createElement('div');actions.className='action-row';
  for(const term of Object.keys(TERMS)){const b=document.createElement('button');b.textContent='Hire · '+TERMS[term].label.toLowerCase()+' $'+c.prices[term].price.toLocaleString();b.disabled=!!reason||full||money<c.prices[term].price;b.title=reason||(full?'The roster holds '+ROSTER_MAX+'.':money<c.prices[term].price?'Not enough in the treasury.':(TERMS[term].minutes/60)+' hours on the campaign clock');b.onclick=()=>{const r=hire(world,c.key,term);message(r.ok?c.name+' signed on: $'+r.price.toLocaleString()+'.':r.error);sync();renderHiring();};actions.append(b);}
  card.append(actions);list.append(card);}
 if(!list.children.length){const p=document.createElement('p');p.textContent='Nobody left at the hall today. New faces at midnight.';list.append(p);}
}
function showHiring(){renderHiring();if(!$('hiring-screen').open)$('hiring-screen').showModal();}
$('hiring-open').onclick=showHiring;$('hiring-close').onclick=()=>$('hiring-screen').close();$('overmap-hiring').onclick=()=>{$('overmap').close();showHiring();};
...
function inventoryStats(u){
 ...
 for(const [label,value]of [['Health',`${u.hp} / ${u.maxHp} HP`],['Action points',`${u.ap} / ${u.maxAp} AP`],['Level',u.level],['Experience',`${u.xp} XP`],['Skill points',u.skillPoints],['Accuracy',u.accuracy],['Medical',`${u.medical} / 100`],['Stealth',`${u.stealth} / 100`],['Sight field',`${u.cone}° · ${sightOf(u).bino}° binocular`],['Stance',STANCES[stanceOf(u)].label]]){
