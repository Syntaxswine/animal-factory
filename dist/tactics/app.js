...
function drawObjects(now){for(const p of Object.values(s.glimpses||{}))if(levelOf(p)===renderLevel)diamond(p.x,p.y,'#e8cb6930','#e8cb69c0',0,.5);for(const [id,p] of Object.entries(s.contacts||{}))if(!s.detected.has(Number(id))&&levelOf(p)===renderLevel){diamond(p.x,p.y,null,'#b4937260',0,.65);textLabel('?',p.x,p.y,'#b49372');}
 ...
 for(const obj of objects){const {x,y,type}=obj,visible=obj.visible??s.visible.has(key(x,y,renderLevel));ctx.globalAlpha=visible?1:.45;
  if(type==='prop'){art.prop(ctx,project,camera.zoom,obj);}
   ...
   if(incapacitated(u)){diamond(x,y,'#bc8f3a77','#edc87a');textLabel(u.casualty==='stable'?'STABLE':u.name+' · '+u.bleedTurns+' turns',x,y,'#ffd98b',11);ctx.globalAlpha=1;continue;}if(!alive(u)&&!flameHolds(u)){diamond(x,y,'#562e2566');drawDeathDrops(ctx,load,u,p,camera.zoom);ctx.font=`${10*camera.zoom}px monospace`;ctx.textAlign='center';ctx.fillStyle='#b5a17c';ctx.fillText('×',p.x-17*camera.zoom,p.y+7*camera.zoom);ctx.globalAlpha=1;continue;}
...
function sync(){ syncClock();if(hover)updateHover(); $('sight-info').textContent=`${selected().cone}° field · ${sightOf(selected()).bino}° binocular · 60 tile identify / 75 tile landscape`;$('turn').disabled=!canControl(s,selected())||!!s.queue.length;selectedIds=new Set([...selectedIds].filter(id=>alive(s.units[id])));selectedIds.add(s.selected);
