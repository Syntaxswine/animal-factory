...
function drawObjects(now){for(const p of Object.values(s.glimpses||{}))if(levelOf(p)===renderLevel)diamond(p.x,p.y,'#e8cb6930','#e8cb69c0',0,.5);for(const [id,p] of Object.entries(s.contacts||{}))if(!s.detected.has(Number(id))&&levelOf(p)===renderLevel){diamond(p.x,p.y,null,'#b4937260',0,.65);textLabel('?',p.x,p.y,'#b49372');}
 ...
 for(const u of s.units)if(u.casualty!=='captured'&&!u.away&&levelOf(u)===renderLevel&&inView(u,b)&&(u.team==='squad'||s.detected.has(u.id)||fallenVisible(s,u,renderLevel)))objects.push({...u,type:'actor',unit:u});
...
function drawFlame({event:e,t,still}){
 const shooter=s.units[e.shooter],target=s.units[e.target];if(!shooter||!target)return;
...
function sync(){ {const settled=resolveRetreat(world);if(settled.ok){adoptMap(settled.state);message('The fight is over for those who stayed. '+squad(s).length+' regrouped in '+s.definition.name+'.');}} syncClock();if(hover)updateHover(); $('sight-info').textContent=`${selected().cone}° field · ${sightOf(selected()).bino}° binocular · 60 tile identify / 75 tile landscape`;$('turn').disabled=!canControl(s,selected())||!!s.queue.length;selectedIds=new Set([...selectedIds].filter(id=>alive(s.units[id])));selectedIds.add(s.selected);
 ...
 $('squad').innerHTML=s.units.filter(u=>u.team==='squad').map(u=>`<button class="squad-card" data-unit="${u.id}" aria-pressed="${selectedIds.has(u.id)}" ${!alive(u)?'disabled':''}><span class="num">0${u.id+1}</span><img alt="" src="${unitArt(u).src}"><div class="info"><strong>${u.name}</strong><small>${alive(u)?`L${levelOf(u)+1} · ${STANCES[stanceOf(u)].label} · ${WEAPONS[u.weapon].short} · ${u.hp} HP${u.burningTurns?' · FIRE '+u.burningTurns+' turns':''}`:incapacitated(u)?(u.casualty==='stable'?'STABILIZED':`BLEEDING · ${u.bleedTurns} turns`):u.casualty==='captured'?'CAPTURED':u.away?'CROSSED · '+world.definitions[u.away.destination].name:'DEAD'}</small><div class="bar"><i style="width:${u.hp/u.maxHp*100}%"></i></div><div class="bar ap"><i style="width:${u.ap/u.maxAp*100}%"></i></div><small>${['explore','won'].includes(s.phase)?'READY':`${u.ap} / ${u.maxAp} AP`}</small></div></button>`).join('');
