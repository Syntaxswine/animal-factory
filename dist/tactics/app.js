...
import {W,H,WEAPONS,inventoryTransfer,turnTo,AIM_ZONES,moveGroup,key,alive,incapacitated,medicalCost,stabilizePreview,stabilize,cutPreview,cutFence,squad,guards,occupant,tile,walkable,createGame,STANCES,stanceOf,setStance,movementNeighbors,navigationPath,pathCost,pathTo,move,stepMovement,previewAttack,attack,equip,reload,endTurn,stepEnemy,canControl} from './engine.js';
...
function drawObjects(now){for(const pile of s.loot||[])if(pile.items.length&&levelOf(pile)===renderLevel&&s.visible.has(key(pile.x,pile.y,renderLevel))){diamond(pile.x,pile.y,'#d5b95870','#f2d78c',0,.35);}
