...
export const DIMENSIONS=Object.freeze({tile:1,floorSpacing:2.12,slab:.12,wall:2,wallThickness:.16,
...
export function buildWorld(map){
 ...
 function tile(x,y,z,terrain){
  ...
  if(!z||!(map.stairs||[]).some(p=>p.x===x&&p.y===y&&p.z===z-1))box(`floor:${x},${y},${z}`,roof?'roof':'floor',roof?.kind||terrain,[x,z*DIMENSIONS.floorSpacing-.06,y],[1,.12,1],{x,y,z});
  if(terrain==='crate')box(`cover:${x},${y},${z}`,'cover','crate-wood',[x,z*DIMENSIONS.floorSpacing+.4,y],[1,.8,1],{x,y,z});
 }
 ...
 for(const [key,kind]of Object.entries(map.edges||{})){
  ...
  const part=(suffix,a,b,bottom,top,material=kind)=>box(`edge:${key}:${suffix}`,'wall',material,
   axis==='e'?[x+.5,z*DIMENSIONS.floorSpacing+(bottom+top)/2,y-.5+(a+b)/2]:[x-.5+(a+b)/2,z*DIMENSIONS.floorSpacing+(bottom+top)/2,y+.5],
   axis==='e'?[.16,top-bottom,b-a]:[b-a,top-bottom,.16],{edge:key});
  if(kind==='fence-cut')continue;
  if(kind==='door'||kind==='doorway-concrete-open'||kind==='door-steel-closed'||kind==='door-wood-closed'){
   part('lintel',0,1,1.65,2);if(kind.endsWith('-closed'))part('door',0,1,0,1.65);continue;
  }
  if(rule?.window){part('sill',0,1,0,.85);part('lintel',0,1,1.55,2);}
  else if(rule?.opaque)part('wall',0,1,0,2);
 ...
 for(const p of map.props||[]){
  ...
  const roof=p.kind.startsWith('roof'),height=roof?.12:PROPS[p.kind].tall?2:.8;
  box(id,roof?'roof':'cover',p.kind,[p.x+(w-1)/2,(p.z||0)*DIMENSIONS.floorSpacing+(roof?-.06:height/2),p.y+(d-1)/2],[w,height,d],{prop:id});
 }
