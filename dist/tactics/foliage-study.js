...
async function start(){
 ...
 let world;function rebuild(){for(const p of map.props)if(p.kind.startsWith('tree-'))p.kind=p.kind.replace(/-large$/,'')+($('trees').value==='large'?'-large':'');world=buildWorld(map);hybrid.rebuild(world,null,0,map);for(const m of hybrid.structures.children){m.castShadow=!m.userData.boxes.every(b=>b.kind==='floor');m.receiveShadow=true;}}
 const params=new URLSearchParams(location.search);$('trees').value=params.get('trees')==='standard'?'standard':'large';if($('trees').value==='large')$('scale').value=host.clientWidth<600?'40':'58';rebuild();
 ...
 let az=GAME_CAMERA.azimuth,el=GAME_CAMERA.elevation;const focus=new T.Vector3(2.6,2.15,1.9);
 ...
 hybrid.onReady=render;for(const id of ['scale','horse'])$(id).onchange=render;$('trees').onchange=()=>{rebuild();render();};$('wire').onchange=()=>{for(const m of hybrid.materials.values())m.wireframe=$('wire').checked;render();};$('reset').onclick=()=>{az=GAME_CAMERA.azimuth;el=GAME_CAMERA.elevation;render();};
 ...
 const resize=new ResizeObserver(render);resize.observe(host);render();window.foliageStudy={hybrid,map,get world(){return world;},horse,render,view(a,e){az=a;el=e;render();},ready:true};
