...
let worker,motion,modelPaint,data,profile,loading=false;
...
async function loadAnimal(id){
 loading=true;window.animalMotionReady=false;$('animal').disabled=true;
 try{const p=ANIMAL_MOTION_CATALOG.find(p=>p.id===id);if(!p)throw Error('Unknown animal '+id);if(worker){scene.remove(worker.root);motion.restore();motion.dispose();modelPaint.dispose();worker.dispose();}
 profile=p;data=await fetch('./'+p.file).then(r=>{if(!r.ok)throw Error('Mesh failed to load');return r.json();});worker=p.create(data,rifle);scene.add(worker.root);modelPaint=await createAnimalPaint(renderer,worker,p,loader);motion=p.unarmed?createHenMotion(worker):createMammalMotion(worker,p);
 ...
 $('visual-status').textContent=ANIMAL_MOTION_FINDINGS[id].status+' — '+ANIMAL_MOTION_FINDINGS[id].note;
 $('pitch').disabled=!!p.unarmed;$('static').href=(id==='horse'?'horse-light':id.startsWith('pig-')?id:id==='dog'?'dog-guard':id+'-worker')+'.html?mesh=10k';
 const url=new URL(location.href);url.searchParams.set('animal',id);history.replaceState(null,'',url);loading=false;$('animal').disabled=false;last=performance.now();render();window.animalMotionReady=true;
...
function render(){
 ...
 $('status').textContent=`${data.triangles.toLocaleString()} character triangles · ${lastD.bones} bones · ${ppu} CSS px/unit · palm error ${Math.max(0,...lastD.contacts.map(c=>c.error)).toExponential(2)}`;
}
...
window.animalMotion={get worker(){return worker;},get motion(){return motion;},renderer,scene,camera,seek,render,loadAnimal,diagnostics:()=>({...lastD,animal:profile.id,pixelsPerUnit:host.clientHeight/(camera.top-camera.bottom),playing,flashPosition:flash.position.toArray(),flashVisible:flash.visible,traceOrigin:Array.from(trace.geometry.attributes.position.array).slice(0,3)})};
