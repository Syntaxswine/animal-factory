...
export class HybridRenderer{
 constructor(onReady=()=>{}){
  ...
  this.structures=new THREE.Group();this.scene.add(this.structures);this.materials=new Map();this.textures=new Map();this.actors=new Map();this.loader=new THREE.TextureLoader();this.onReady=onReady;this.boxGeometry=new THREE.BoxGeometry(1,1,1);this.diagnostics=[];
 }
 material(kind){if(this.materials.has(kind))return this.materials.get(kind);const p=surfacePixels(kind),t=new THREE.DataTexture(p.data,p.width,p.height);t.colorSpace=THREE.SRGBColorSpace;t.wrapS=t.wrapT=THREE.RepeatWrapping;t.magFilter=THREE.NearestFilter;t.minFilter=THREE.LinearMipmapLinearFilter;t.generateMipmaps=true;t.needsUpdate=true;const material=new THREE.MeshStandardMaterial({map:t,roughness:1});
  material.onBeforeCompile=shader=>{shader.vertexShader=shader.vertexShader.replace('#include <uv_vertex>','#include <uv_vertex>\n#ifdef USE_MAP\n vec3 surfaceWorld=(instanceMatrix*vec4(position,1.0)).xyz; vMapUv=abs(normal.y)>.5?surfaceWorld.xz:abs(normal.x)>.5?surfaceWorld.zy:surfaceWorld.xy;\n#endif');};this.materials.set(kind,material);return material;
 }
 rebuild(world,seen,level){
  const previous=this.chunks||new Map(),next=new Map();
  const groups=new Map();
  for(const box of world.boxes){const z=box.source.z??(box.source.edge?Number(box.source.edge.split(':')[3]||0):0);if(z>level||!known(box,seen))continue;const material=materialKind(box),groupKey=`${material}:${Math.floor(box.center[0]/16)},${Math.floor(box.center[2]/16)}`;if(!groups.has(groupKey))groups.set(groupKey,{material,boxes:[]});groups.get(groupKey).boxes.push(box);}
  const matrix=new THREE.Matrix4(),rotation=new THREE.Quaternion(),position=new THREE.Vector3(),scale=new THREE.Vector3();
  for(const [key,{material,boxes}]of groups){const old=previous.get(key),same=old&&old.userData.boxes.length===boxes.length&&boxes.every((b,i)=>{const a=old.userData.boxes[i];return a.id===b.id&&a.center.every((n,j)=>n===b.center[j])&&a.size.every((n,j)=>n===b.size[j]);});if(same){next.set(key,old);previous.delete(key);continue;}
   const mesh=new THREE.InstancedMesh(this.boxGeometry,this.material(material),boxes.length);for(const [i,b]of boxes.entries()){position.fromArray(b.center);scale.fromArray(b.size);matrix.compose(position,rotation,scale);mesh.setMatrixAt(i,matrix);}mesh.instanceMatrix.needsUpdate=true;mesh.computeBoundingSphere();mesh.userData.boxes=boxes;this.structures.add(mesh);next.set(key,mesh);}
 ...
 draw(ctx,state,view,width,height,level,{editor=false}={}){
  ...
  const seen=editor?null:state.seen,rebuild=this.world!==world||this.seenCount!==seen?.size||this.level!==level;if(rebuild){this.rebuild(world,seen,level);this.world=world;this.seenCount=seen?.size;this.level=level;}
  ...
  this.prune();this.renderer.render(this.scene,camera);ctx.drawImage(this.renderer.domElement,0,0,width,height);this.drawTimes??=[];this.drawTimes.push(performance.now()-drawStart);if(this.drawTimes.length>120)this.drawTimes.shift();return picking;
 }
 ...
 dispose(){for(const mesh of this.structures.children)mesh.dispose();for(const mesh of this.actors.values()){mesh.geometry.dispose();mesh.material.dispose();}for(const t of this.textures.values())t.dispose();this.textures.clear();this.actors.clear();for(const m of this.materials.values()){m.map.dispose();m.dispose();}this.materials.clear();this.chunks?.clear();this.world=null;this.editorWorld=null;this.scene.clear();this.boxGeometry.dispose();this.renderer.dispose();}
}
