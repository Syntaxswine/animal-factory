...
import {W,H,stampRoom,tileKey,levelOf,terrainAt,setTerrain,addStairs,MAX_GUARDS,passable,blockedEdge,inBounds} from './maps.js';
...
export function applyBrush(editor,tool,x,y,edge,options={}) {
 ...
 if(tool==='stairs'||tool==='ladder'){if(z===2)return 'Stairs must begin on level 1 or 2.';if([z,z+1].some(l=>terrainAt(m,x,y,l)==='crate'||propAt(m,x,y,l)))return 'Remove crates from both stair endpoints first.';addStairs(m,x,y,z,tool==='ladder'?'ladder':'stairs');return '';}
 ...
 if(['yard','floor','crate','void','water','bridge'].includes(tool)){if(tool==='water'&&z!==0)return 'Water belongs on the ground level.';if(['crate','void','water'].includes(tool)&&(guard||start>=0||exit||propAt(m,x,y,z)||m.stairs.some(p=>p.x===x&&p.y===y&&(p.z===z||p.z+1===z))))return 'Move starts, travel markers and stairs before blocking this tile.';setTerrain(m,x,y,z,tool);return '';}
