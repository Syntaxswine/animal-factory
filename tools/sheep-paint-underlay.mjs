...
try {
 const page=await browser.newPage();await page.goto('http://127.0.0.1:4389/tactics/sheep-worker.html?stage=grey');await page.waitForFunction(()=>window.lightHorseReady);
 for(const kind of ['underlay']){
  const png=await page.evaluate(async kind=>{
   const T=await import('./vendor/three.module.js'),{PAINT_FRAME:f}=await import('./horse-model-paint.js'),sheep=sheepVariants['28k'].horse;
   const scene=new T.Scene();
   for(let i=0;i<sheep.parts.length;i++){
    if(![1,8].includes(i))continue;
    const g=sheep.parts[i].geometry.clone();g.setAttribute('position',g.getAttribute('paintPosition'));g.setAttribute('normal',g.getAttribute('paintNormal'));scene.add(new T.Mesh(g,sheep.grey));
   }
  ...
  fs.writeFileSync(new URL('../docs/tactics/hybrid-review/sheep-worker/'+kind+'-paint-reference.png',import.meta.url),Buffer.from(png,'base64'));
 }
