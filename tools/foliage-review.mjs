...
try{
 ...
 await p.locator('#trees').selectOption('standard');await p.evaluate(()=>foliageStudy.view(Math.PI/4,Math.PI/6));
 for(const scale of ['125','58']){await p.locator('#scale').selectOption(scale);await p.locator('#scene').screenshot({path:fileURLToPath(new URL(`standard-${scale}.png`,out))});}
 await p.locator('#trees').selectOption('large');
 const checks=await p.evaluate(async()=>{
  const {buildWorld}=await import('./hybrid-world.js'),{hybrid:h}=foliageStudy;
  const map={terrain:Array.from({length:32},()=>Array(32).fill('ground-grass')),upper:[{'1,1':'ground-grass'}],props:[],edges:{},stairs:[]};for(let y=1;y<32;y+=4)for(let x=1;x<32;x+=4)map.props.push({x,y,kind:((x+y)%8===2?'tree-pine':'tree-broadleaf')+(x%8===1?'-large':'')});
 ...
 await p.setViewportSize({width:390,height:844});await p.locator('#scale').selectOption('40');await p.locator('#reset').click();await p.screenshot({path:fileURLToPath(new URL('mobile.png',out)),fullPage:true});
