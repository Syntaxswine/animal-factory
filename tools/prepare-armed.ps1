param([string]$SourceList='art/armed-sources.json')
...
$out=Join-Path $root 'dist/assets/characters/armed'
$sourceDir=Join-Path $root 'art/source/armed'
...
foreach($entry in $entries){
 $name=$entry.species+'-'+$entry.weapon+'.png'
 ...
 $b=[SpriteBounds]::Find($img)
 $scale=236.0/($b[3]-$b[1]+1)
 $left=128-($b[4]-$b[0])*$scale
 $w=($b[2]-$b[0]+1)*$scale
 if($left -lt 2 -or ($left+$w) -gt 254){throw "Weapon exceeds frame: $name"}
 $dest=[System.Drawing.Bitmap]::new(256,256,[System.Drawing.Imaging.PixelFormat]::Format32bppArgb)
 $g=[System.Drawing.Graphics]::FromImage($dest)
 $g.InterpolationMode=[System.Drawing.Drawing2D.InterpolationMode]::HighQualityBicubic
 $g.DrawImage($img,[System.Drawing.RectangleF]::new($left,8,$w,236),[System.Drawing.RectangleF]::new($b[0],$b[1],$b[2]-$b[0]+1,$b[3]-$b[1]+1),[System.Drawing.GraphicsUnit]::Pixel)
