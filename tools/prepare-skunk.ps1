param([string]$SourceList='art/skunk-sources.json')
...
$out=Join-Path $root 'dist/assets/characters'
$sourceDir=Join-Path $root 'art/source/skunk'
...
foreach($entry in $entries){
 $name=$entry.outfit+'-'+$entry.weapon+'-'+$entry.stance+'.png'
 $frameWidth=if($entry.stance -eq 'prone'){512}else{384}
 if($entry.outfit -eq 'red-hats'){
  $relative=if($entry.weapon -eq 'hands' -and $entry.stance -eq 'standing'){'red-hats/skunk-idle.png'}else{'red-hats/skunk-'+$entry.weapon+'-'+$entry.stance+'.png'}
  if($entry.weapon -eq 'hands' -and $entry.stance -eq 'standing'){$frameWidth=256}
 }elseif($entry.stance -ne 'standing'){$relative='stances/skunk-'+$entry.weapon+'-'+$entry.stance+'.png'}elseif($entry.weapon -eq 'hands'){$relative='skunk-idle.png';$frameWidth=192}else{$relative='armed/skunk-'+$entry.weapon+'.png';$frameWidth=256}
 ...
 $left=$frameWidth/2-($b[2]-$b[0]+1)*$scale/2
 $w=($b[2]-$b[0]+1)*$scale
 if($left -lt 2 -or ($left+$w) -gt ($frameWidth-2)){throw "Weapon exceeds frame: $name"}
