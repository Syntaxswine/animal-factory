...
function save(file,parts,sourceTriangles){const data={schema:1,species:'donkey',source:'Donkey-specific head, long ears, mane and tail; fitted jacket and work trousers on shared worker rig',sourceTriangles,triangles:parts.reduce((n,p)=>n+p.triangles,0),parts};fs.writeFileSync(new URL('../dist/tactics/'+file,import.meta.url),JSON.stringify(data)+'\n');console.log(file,data.triangles,parts.map(p=>[p.name,p.triangles,p.errorWorld]));return data;}
