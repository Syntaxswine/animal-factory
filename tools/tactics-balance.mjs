...
export function simulate(seed,{map,maxActions=3000,...options}={}){
 const s=createGame(seed,map),bot=createSquadBot(options);let actions=0,stalled=false;
...
if(process.argv[1]?.endsWith('tactics-balance.mjs')){
 ...
 const results=Array.from({length:count},(_,i)=>simulate(seed+i,{map,orders,quietOpening:has('--quiet-opening'),maxActions:has('--max-actions')?Number(value('--max-actions')):map?20000:3000}));
