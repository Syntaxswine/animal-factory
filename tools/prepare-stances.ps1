param([string]$SourceList='art/stance-sources.json')
...
$out=Join-Path $root 'dist/assets/characters/stances'
$sourceDir=Join-Path $root 'art/source/stances'
...
foreach($entry in $entries){
 ...
 $targetHeight=if($entry.stance -eq 'kneeling'){176}else{96}
