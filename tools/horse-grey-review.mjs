...
try{
 ...
 await page.locator('#wire').uncheck();await page.locator('#view').selectOption('gameplay');await page.screenshot({path:directory+'/gameplay-size.png'});
 const comparison=await page.evaluate(()=>greyHorseDiagnostics().comparison);assert.ok(Math.abs(comparison.pixelsPerUnit-58)<1e-8);assert.equal(comparison.viewportHeight,240);assert.ok(Math.abs(comparison.span-240/58)<1e-8);
 await page.locator('#silhouette').check();await page.screenshot({path:directory+'/gameplay-silhouettes.png'});
 ...
 fs.writeFileSync(directory+'/checks.json',JSON.stringify({browser:browser.version(),errors,...d,comparison},null,2)+'\n');console.log(JSON.stringify(d,null,2));
