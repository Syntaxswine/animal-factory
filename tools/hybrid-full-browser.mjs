...
try{
 for(const [name,url]of [['legacy-game','index.html?map=custom'],['hybrid-game','index.html?map=custom&renderer=hybrid'],['legacy-editor','editor.html'],['hybrid-editor','editor.html?renderer=hybrid']]){
  ...
  page.on('response',r=>{if(r.status()>=400&&!r.url().endsWith('favicon.ico'))errors.push(r.status()+' '+r.url());});await page.addInitScript(map=>sessionStorage.setItem('red-shift-playtest',JSON.stringify(map)),map);const start=Date.now();await page.goto('http://127.0.0.1:4389/tactics/'+url,{waitUntil:'networkidle',timeout:60000});
  ...
  await page.screenshot({path:fileURLToPath(new URL(`../docs/tactics/hybrid-review/${name}.png`,import.meta.url))});results.push({name,browser:browser.version(),viewport:{width:1440,height:1000},loadMs,...timing,activePanFrameMs:active,undoInteractionMs:edits,stats:info.stats});await page.close();
 }
 fs.writeFileSync(new URL('../docs/tactics/hybrid-review/browser-benchmark.json',import.meta.url),JSON.stringify(results,null,2));console.log(JSON.stringify(results,null,2));
