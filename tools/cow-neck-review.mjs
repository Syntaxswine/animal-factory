...
const dir=new URL('../docs/tactics/hybrid-review/cow-worker/',import.meta.url);
...
try{
 ...
 await page.goto('http://127.0.0.1:4424/tactics/cow-worker.html?mesh=10k');
 ...
 for(const mesh of ['10k','28k']){
  ...
  for(const pose of ['neutral','carry']){
   ...
   for(const view of ['front','side','back','three','game']){
    ...
    await page.screenshot({path:fileURLToPath(new URL(`neck-${mesh}-${pose}-${view}.png`,dir)),clip:{x:835,y:300,width:275,height:235}});
   }
  }
 }
 const regressions=[];
 for(const species of ['horse-light','goat-worker','bull-worker']){
  await page.goto(`http://127.0.0.1:4424/tactics/${species}.html?mesh=10k`);
 ...
 fs.writeFileSync(new URL('neck-checks.json',dir),JSON.stringify({browser:browser.version(),errors,checks,regressions},null,2)+'\n');
 console.log('Cow neck review: 20 close-up views and 3 high-DPI character regressions, no browser errors.');
