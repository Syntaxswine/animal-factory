...
function scarfWrap(){const p=[],ix=[],n=24,rows=3,count=n*rows;for(let side=0;side<2;side++)for(let j=0;j<rows;j++)for(let i=0;i<n;i++){const t=j/2,a=i*2*Math.PI/n,inset=side?.010:0,rx=(.153-.027*t)*(1-.35*Math.pow(Math.max(0,Math.cos(a)),4)),rz=.179-.053*t;const fold=.002*Math.sin(a*3+t*5)*Math.sin(t*Math.PI);p.push(-.050+(rx-inset+fold)*Math.cos(a),1.275+.040*t-.016*t*Math.pow(Math.max(0,Math.cos(a)),4),(rz-inset+fold)*Math.sin(a));}for(let side=0;side<2;side++)for(let j=0;j<rows-1;j++)for(let i=0;i<n;i++){const a=side*count+j*n+i,b=side*count+j*n+(i+1)%n,c=b+n,d=a+n;if(side)ix.push(a,b,d,b,c,d);else ix.push(a,d,b,b,d,c);}for(let i=0;i<n;i++){const a=i,b=(i+1)%n,c=2*n+i,d=2*n+(i+1)%n;ix.push(a,b,a+count,b,b+count,a+count,c,c+count,d,d,c+count,d+count);}const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(p,3));g.setIndex(ix);g.computeVertexNormals();return g;}
add('fitted neckerchief wrap',scarfWrap());
// Stable part order is also the paint ownership order shared with the horse.
const order=['connected shirt and sleeves','connected work trousers seat and legs','forearm and hand -1','exposed cloven hoof -1','forearm and hand 1','exposed cloven hoof 1','unified sheep skull ears and wool','short woolly tail','fitted waistcoat with pointed hem','fitted neckerchief wrap'];geometries.sort((a,b)=>order.indexOf(a.name)-order.indexOf(b.name));
