...
for($page=0;$page -lt 3;$page++){
 ...
 try {
  ...
  for($col=0;$col -lt 3;$col++){for($row=0;$row -lt 3;$row++){
   ...
   $img=[System.Drawing.Bitmap]::new((Join-Path $root ('dist/assets/characters/weapon-expansion/normal/'+$animal+'-flamethrower-'+$stance+'.png')))
  ...
  $path=Join-Path $root ('art/flamethrower-review-'+($page+1)+'.png')
